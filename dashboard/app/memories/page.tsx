...
export default function Memories() {
  ...
  const filteredMems = mems.filter((m) => {
    const matchesSearch =
      !srch || m.content.toLowerCase().includes(srch.toLowerCase());
    return matchesSearch;
  });

  const sectorCounts = mems.reduce(
    (acc, m) => {
    ...
    <div className="min-h-screen" suppressHydrationWarning>
      ...
      >
        ...
        <fieldset className="space-y-2 border border-stone-900 rounded-3xl px-4 pb-4 pt-1 flex justify-center items-center col-span-2">
          ...
          <div className="relative flex items-center text-sm w-full">
            <svg
              ...
              strokeWidth="1.5"
              stroke="currentColor"
              className="size-4 absolute left-3"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            <button
              ...
              className="absolute right-1 p-1 px-3 bg-stone-900 rounded-lg hover:bg-stone-800"
            >
              /
            </button>
            <input
        ...
        <fieldset className="space-y-2 border border-stone-900 rounded-3xl px-5 pb-4 pt-4 flex justify-center items-center col-span-3">
          ...
          >
            <div
              className={`rounded-xl border border-sky-500/15 p-3 text-sky-500 bg-sky-500/10 text-center`}
              suppressHydrationWarning
            >
            ...
            <div
              className={`rounded-xl border border-amber-500/15 p-3 text-amber-500 bg-amber-500/10 text-center`}
              suppressHydrationWarning
            >
              <div className="text-2xl font-bold" suppressHydrationWarning>
            ...
            <div
              className={`rounded-xl border border-emerald-500/15 p-3 text-emerald-500 bg-emerald-500/10 text-center`}
              suppressHydrationWarning
            >
              <div className="text-2xl font-bold" suppressHydrationWarning>
            ...
            <div
              className={`rounded-xl border border-rose-500/15 p-3 text-rose-500 bg-rose-500/10 text-center`}
              suppressHydrationWarning
            >
              <div className="text-2xl font-bold" suppressHydrationWarning>
            ...
            <div
              className={`rounded-xl border border-purple-500/15 p-3 text-purple-500 bg-purple-500/10 text-center`}
              suppressHydrationWarning
            >
              <div className="text-2xl font-bold" suppressHydrationWarning>
        ...
        <div className="col-span-6 rounded-xl border border-stone-900 p-4">
          ...
          {error && (
            <div className="text-rose-400 text-center py-8">Error: {error}</div>
          )}
          {!loading && !error && (
                ...
                filteredMems.map((mem) => (
                  ...
                  >
                    <div className="flex items-start space-x-3 flex-1">
                      <div
                        className={`rounded-xl border border-${sectorColors[mem.primary_sector]}-500/15 p-2 text-${sectorColors[mem.primary_sector]}-500 bg-${sectorColors[mem.primary_sector]}-500/10`}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 24 24"
                      ...
                      <div className="flex flex-col flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {mem.tags?.map((tag) => (
                            <span
                              key={tag}
                              className="bg-stone-800 rounded px-2 py-0.5 text-xs text-stone-400"
                            >
                              {tag}
                            </span>
                          ))}
...
function AddMemoryModal({
  ...
  onClose: () => void;
  onAdd: (content: string, sector: string, tags: string) => void;
}) {
