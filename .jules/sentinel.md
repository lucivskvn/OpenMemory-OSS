...
**Vulnerability:** The MCP tool call handlers (`openmemory_query`, `openmemory_store`, and `openmemory_list`) in `packages/openmemory-py/src/openmemory/ai/mcp.py` extracted `user_id` directly from caller-supplied tool arguments without resolving tenant identity via `_resolve_mcp_tenant`. Unauthenticated sessions or cross-tenant callers could query, store, or list memories under arbitrary claimed `user_id` identities.
**Learning:** While `openmemory_get` and `openmemory_delete` were protected via `_get_verified_memory`, secondary tool handlers (`query`, `store`, `list`) relied on raw argument extraction, creating a severe multi-tenant isolation discrepancy.
**Prevention:** Always invoke `_resolve_mcp_tenant(mem, args)` across all tool call handlers in MCP server adapters before executing database reads, writes, or searches, ensuring requests fail closed on unauthenticated sessions or tenant mismatches.
