    ...
    return (
      <div className="min-h-screen bg-black text-[#e6e6e6] flex items-center justify-center">
        <div className="text-center">
          <div className="text-stone-400 text-xl mb-2">
            Loading Dashboard...
          </div>
          ...
          <div className="flex gap-2">
            {timePeriods.map((period) => (
              <button
                ...
                className={`rounded-xl p-2 pl-4 flex items-center justify-center transition-colors ${
        ...
        <div className="grid grid-cols-4 gap-4 mb-6 xl:grid-cols-8">
          ...
          <StatCard
            ...
            statusColor={getStatusColor(
              healthMetrics.errors > 10 ? 'Critical' : 'Stable',
            )}
          />
          <StatCard
            label="Memory Usage"
          ...
          <StatCard
            ...
            status={
              systemHealth.embeddingProvider === 'router_cpu'
                ? 'Optimized'
                : 'Standard'
            }
            statusColor={getStatusColor(
          ...
          <StatCard
            ...
            statusColor={getStatusColor(
              systemHealth.simdEnabled ? 'Active' : 'Inactive',
            )}
          />
        </div>
          ...
          <div className="bg-transparent rounded-xl p-6 border border-[#27272a] hover:border-zinc-600 transition-colors duration-200">
                ...
                <div className="space-y-1 max-h-64 overflow-y-auto">
                  {telemetry.length > 0 ? (
                    telemetry.map((t, idx) => (
                      <div
                        key={idx}
                        className="bg-transparent rounded px-2 py-2 grid grid-cols-6 gap-2 text-xs text-[#9ca3af] hover:bg-transparent border border-transparent transition-colors"
                      >
                        <span className="text-[#6b7280]">
                          {formatTime(t.ts || now, { timeOnly: true })}
                        </span>
                        <span className="col-span-1">
                          {t.user_id || 'anonymous'}
                        </span>
                        <span className="text-[#9ca3af]">
                          {t.embedding_mode || 'n/a'}
                        </span>
                        <span>{t.duration_ms}</span>
                        <span>
                          {Array.isArray(t.memory_ids)
                            ? t.memory_ids.length
                            : t.memory_ids
                              ? t.memory_ids.length
                              : 0}
                    ...
                    <div className="text-center text-stone-500 py-8">
                      No telemetry yet. Start streaming answers to generate
                      telemetry.
                    </div>
                  )}
                </div>
            ...
            <div className="grid grid-cols-5 gap-2 mt-4">
              ...
              <div className="bg-transparent rounded p-2 border border-[#27272a] hover:border-zinc-600 transition-colors duration-200 text-center">
                ...
                <p className="text-lg font-bold text-[#f4f4f5]">
                  {formatNumber(qpsStats.total || 0)}k
                </p>
              </div>
              <div className="bg-transparent rounded p-2 border border-[#27272a] hover:border-zinc-600 transition-colors duration-200 text-center">
          ...
          <div className="bg-transparent rounded-xl p-6 border border-[#27272a] hover:border-zinc-600 transition-colors duration-200">
            ...
            <div className="grid grid-cols-5 gap-2 mt-4">
              <div className="bg-transparent rounded p-2 border border-[#27272a] hover:border-zinc-600 transition-colors duration-200 text-center">
                ...
                <p className="text-lg font-bold text-[#22c55e]">
                  {healthMetrics.decayStability || '0'}%
                </p>
              </div>
              <div className="bg-transparent rounded p-2 border border-[#27272a] hover:border-zinc-600 transition-colors duration-200 text-center">
              ...
              <div className="bg-transparent rounded p-2 border border-[#27272a] hover:border-zinc-600 transition-colors duration-200 text-center">
                ...
                <p className="text-lg font-bold text-[#60a5fa]">
                  {maintenanceStats.reflections || 0}
                </p>
              </div>
              <div className="bg-transparent rounded p-2 border border-[#27272a] hover:border-zinc-600 transition-colors duration-200 text-center">
                <p className="text-xs text-[#8a8a8a] mb-1">Consol.</p>
                <p className="text-lg font-bold text-[#c084fc]">
                  {maintenanceStats.consolidations || 0}
                </p>
              </div>
              <div className="bg-transparent rounded p-2 border border-[#27272a] hover:border-zinc-600 transition-colors duration-200 text-center">
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className="space-y-2 text-sm text-[#8a8a8a]">
              ...
              <div className="flex justify-between border-b border-[#27272a] pb-2">
                <span>Uptime</span>
                <span className="text-[#e6e6e6]">
                  {systemHealth.uptimeDays || 0}d{' '}
                  {systemHealth.uptimeHours || 0}h
                </span>
              </div>
            </div>
            <div className="space-y-2 text-sm text-[#8a8a8a]">
              ...
              <div className="flex justify-between border-b border-[#27272a] pb-2">
                <span>Embedding Mode</span>
                <span className="text-[#e6e6e6]">
                  {embeddingHealth?.mode || 'N/A'}
                </span>
              </div>
              <div className="flex justify-between border-b border-[#27272a] pb-2">
                <span>SIMD Global</span>
                <span
                  className={`${systemHealth?.simdGlobal ? 'text-[#22c55e]' : 'text-[#f87171]'}`}
                >
              ...
              <div className="flex justify-between border-b border-[#27272a] pb-2">
                ...
                <span
                  className={`${systemHealth?.simdRouter ? 'text-[#22c55e]' : 'text-[#f87171]'}`}
                >
              ...
              <div className="flex justify-between border-b border-[#27272a] pb-2">
                ...
                <span
                  className={`${systemHealth?.simdEnabled ? 'text-[#22c55e]' : 'text-[#f87171]'}`}
                >
