...
test(
  'postgres q parity: placeholders, ordering, null handling',
  async () => {
    ...
    try {
      ...
      const row = await get(`select $1 as a, $1 as b, $2 as id`, [
        'reused',
        testId,
      ]);
      ...
      const inRows = await all(
        `select id,content from ${memTable} where id in ($1,$2) order by id`,
        [idA, idB],
      );
