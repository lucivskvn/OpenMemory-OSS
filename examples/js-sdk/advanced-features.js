...
async function advancedExample() {
  ...
  try {
    ...
    const projectMemory = await client.add(
      'Started working on the OpenMemory project',
      {
    ...
    const learningMemory = await client.add(
      'Learned about vector databases and embeddings',
      {
    ...
    console.log(
      'Before reinforcement salience:',
      beforeReinforce.matches[0]?.salience,
    );
    ...
    console.log(
      'After reinforcement salience:',
      afterReinforce.matches[0]?.salience,
    );
    ...
    console.log(
      `✅ Semantic sector results: ${semanticResults.matches.length}`,
    );
    ...
    const tempMemory = await client.add(
      'This is a temporary memory for testing deletion',
    );
    ...
    addedBatch.forEach((memory) => {
      sectorCount[memory.primary_sector] =
        (sectorCount[memory.primary_sector] || 0) + 1;
    });
