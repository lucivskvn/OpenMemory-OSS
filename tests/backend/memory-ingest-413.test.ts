...
test('POST /memory/ingest maps ERR_FILE_TOO_LARGE to 413', async () => {
  ...
  const dbPath = path.join(
    tmpDir,
    `openmemory-memory-ingest-413-${process.pid}-${Date.now()}.sqlite`,
  );
  ...
  const actualPort =
    (serverInfo as any).port || Number(process.env.OM_PORT) || port;
  ...
  try {
    (ingestMod as any).setIngestDocumentForTests(null);
  } catch (e) {}
});
...
test('POST /memory/ingest maps UnsupportedContentTypeError to 415', async () => {
  ...
  const dbPath = path.join(
    tmpDir,
    `openmemory-memory-ingest-415-${process.pid}-${Date.now()}.sqlite`,
  );
  ...
  if (typeof (ingestMod as any).setIngestDocumentForTests === 'function') {
    (ingestMod as any).setIngestDocumentForTests(async () => {
      ...
      throw new (extractMod as any).UnsupportedContentTypeError(
