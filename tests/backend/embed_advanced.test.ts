...
function installDefaultProviders(mod: any) {
    try {
        ...
        const defaultBatchProvider = async (texts: any[]) => texts.map(() => Array.from({ length: dim }, (_, i) => (i + 1) / dim));
        if (typeof mod.__setTestProvider === 'function') mod.__setTestProvider(defaultProvider);
        else if (mod && mod.__TEST) mod.__TEST.provider = defaultProvider;
        if (typeof mod.__setTestBatchProvider === 'function') mod.__setTestBatchProvider(defaultBatchProvider);
    ...
    try {
        ...
        if (embedMod && embedMod.__TEST) {
            ...
            if (typeof embedMod.__TEST.reset === 'function') {
                try { embedMod.__TEST.reset(); } catch (e) { }
            }
        }
    } catch (e) {
