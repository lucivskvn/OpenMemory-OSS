        ...
        listen(port: number, callback?: () => void) {
            const fetch = async (req: Request): Promise<Response> => {
                ...
                if (req.headers.get("upgrade") === "websocket") {
                    const { route, params } = (app as any).matchWsRoute(
                        url.pathname,
                    );
                    if (route) {
                        ...
                        if (
                            ...
                            logger.info(
                                ...
                                `WS upgraded for ${url.pathname}`,
                            );
                            return undefined as any;
                        }
                        logger.error(
                            { component: "SERVER", path: url.pathname },
                            `WS upgrade failed for path: ${url.pathname}`,
                        );
                ...
                const { handler, params } = (app as any).matchRoute(
                    req.method,
                    url.pathname,
                );
                ...
                if (contentType.includes("application/json")) {
                    const maxSize =
                        ...
                        (env && (env.max_payload_size as any)) ||
                        1000000;
                ...
                try {
                    if (
                        ...
                        if (
                            !rawText &&
                            typeof (req as any).clone === "function"
                        ) {
                            try {
                        ...
                        if (
                            !rawText &&
                            typeof (req as any).text === "function"
                        ) {
                            try {
                        ...
                        if (rawText) {
                            try {
                                ...
                                try {
                                    (req as any).body = (ctx as any).body;
                                } catch (_) {}
                            } catch (_e) {
                ...
                const runner = async (index: number): Promise<Response> => {
                    if (index < middlewares.length) {
                        return middlewares[index](req, ctx, () =>
                            runner(index + 1),
                        );
                    }
                    ...
                    const globalLegacyMode =
                        process.env.OM_LEGACY_HANDLER_MODE === "true";
                    ...
                    if (isLegacyMode) {
                        const { resShim, headers, getStatusBody } =
                            makeResShim();
                        try {
                    ...
                    logger.error(
                        { component: "SERVER", path: url.pathname },
                        "Handler returned undefined in modern mode. To maintain compatibility mark legacy handlers with handler.__legacy = true or set OM_LEGACY_HANDLER_MODE=true. Otherwise ensure handlers return a Response or serializable value.",
                    );
                    return new Response(
                        JSON.stringify({
                            error: "handler_returned_undefined",
                            message:
                                "Handler returned undefined in modern (req, ctx) mode. Mark legacy handlers with handler.__legacy = true or set OM_LEGACY_HANDLER_MODE=true to opt-in to legacy (req, res) behavior, or update the handler to return a Response or a serializable value.",
                        }),
                        {
                            status: 500,
                            headers: { "Content-Type": "application/json" },
                ...
                try {
                    ...
                    logger.error(
                        { component: "SERVER", err: e, path: url.pathname },
            ...
            const websocket: any = {
                ...
                message(ws: any, msg: any) {
                    ...
                    if (route) {
                        ...
                        if (typeof route.handler.message === "function")
                            route.handler.message(ws, msg);
                    }
                },
                close(ws: any, code: any, reason: any) {
                    ...
                    if (route) {
                        ...
                        if (typeof route.handler.close === "function")
            ...
            for (const route of routes) {
                ...
                for (let i = 0; i < routeParts.length; i++) {
                    if (routeParts[i].startsWith(":")) {
                        params[routeParts[i].slice(1)] = decodeURIComponent(
                            pathParts[i],
                        );
