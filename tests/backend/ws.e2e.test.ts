...
async function waitForProcessOutput(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    proc.on('exit', () => {
      ...
      const msg = stderr
        ? `server process exited prematurely; helper stderr: ${stderr}`
...
describe('ws e2e', () => {
  it('server process upgrades websocket and sends params', async () => {
    ...
    const cand1 = path.resolve(
      process.cwd(),
      'tests',
      'backend',
      'e2e_server.mjs',
    );
    const cand2 = path.resolve(
      process.cwd(),
      '..',
      'tests',
      'backend',
    ...
    const proc = spawn(bunCmd, [script], {
      cwd: process.cwd(),
