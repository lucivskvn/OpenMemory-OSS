...
describe('Stream telemetry persistence', () => {
  it('persists telemetry via /dashboard/telemetry/stream', async () => {
    ...
    const res = await fetch(
      `http://127.0.0.1:${port}/dashboard/telemetry/stream`,
      {
        method: 'POST',
    ...
    const row = await get_async(
      'select id, user_id, embedding_mode, duration_ms, memory_ids, query from stream_telemetry where id=?',
      ['telemetry-test-1'],
    );
    ...
    const getRes = await fetch(
      `http://127.0.0.1:${port}/dashboard/telemetry?limit=10`,
    );
    ...
    const notFoundRes = await fetch(
      `http://127.0.0.1:${port}/dashboard/telemetry?limit=10&embedding_mode=local`,
    );
    expect(notFoundRes.status).toBe(200);
    const notFound = await notFoundRes.json();
    const foundNot = notFound.telemetry.find(
      (t: any) => t.id === 'telemetry-test-1',
    );
  ...
  it('requires admin key when OM_ADMIN_API_KEY configured', async () => {
    ...
    const routeMod: any = await import(
      '../../backend/src/server/routes/dashboard'
    );
    ...
    const getOk = await fetch(`http://127.0.0.1:${port}/dashboard/telemetry`, {
      headers: { 'x-admin-key': adminPlain },
    });
    expect(getOk.status).toBe(200);
    const data = await getOk.json();
    const found = data.telemetry.find(
      (t: any) => t.id === 'telemetry-test-admin-1',
    );
    expect(found).toBeTruthy();
