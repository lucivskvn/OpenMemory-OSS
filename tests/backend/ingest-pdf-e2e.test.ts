...
import { ingestDocumentFromFile, ingestDocument } from "../../backend/src/ops/ingest";
...
describe("ingestDocumentFromFile end-to-end PDF ingestion", () => {
    ...
    (ingestDocument as any)._add_hsg_memory = async (_content: any, _t: any, _m: any, _user?: any) => ({ id: 'mock-root' });

    // Call the real file ingestion helper which will detect mime and call extract
    const res = await ingestDocumentFromFile(fixturePath, 'application/pdf', undefined, { force_root: false }, 'test-user-e2e');
