...
describe("Phase 113: Temporal Analytics & Aggregation", () => {
    ...
    beforeAll(async () => {
        ...
        reloadConfig();
        // Cleanup existing test data for this subject/user
        // await runAsync(`DELETE FROM ${TABLES.temporal_facts} WHERE subject = ? AND user_id = ?`, [subject, userId]);
        // With :memory:, it is empty.
    });
