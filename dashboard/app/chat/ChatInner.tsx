...
export default function ChatInner() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        console.warn(
          'Embedding config fetch failed, falling back to synthetic mode:',
          e,
        );
        setEmbeddingConfig({
          kind: 'synthetic',
  ...
  const derived = useMemo(() => {
    if (chatMessages.length === 0)
      return { telemetry: null, memories: [] as MemoryReference[] };
  ...
  return (
    <div className="flex flex-col min-h-screen w-full" suppressHydrationWarning>
      <div
        className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8 mt-6 mb-16"
              ...
              {chatMessages.map((m: any, i: number) => {
                ...
                if (m.role === 'assistant') {
                  const cleanContent = text
                    .replace(
                      /\[\[OM_TELEMETRY\]\].*\[\[\/OM_TELEMETRY\]\]/g,
                      '',
                    )
                    .replace(/\[\[OM_MEMORIES\]\].*\[\[\/OM_MEMORIES\]\]/g, '');
                  return (
                  ...
                  <div className="bg-stone-900/70 border border-zinc-800 rounded-2xl px-4 py-3">
                    <div className="flex gap-2 items-center text-stone-400">
                      <div className="flex gap-1.5">
                        <div
                          className="w-2 h-2 rounded-full bg-stone-600 animate-bounce"
                  ...
                  <div className="inline-block max-w-[85%] bg-red-900/70 border border-red-800 rounded-2xl px-4 py-3">
                    <div className="text-red-200">
                      ...
                      <div className="text-sm text-red-300 mt-1">
                        Configure OPENAI_API_KEY to enable chat generation.
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="mb-5">
              <div className="rounded-2xl bg-stone-950/80 border border-zinc-900 px-4 py-3 mb-3">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-stone-100 font-semibold tracking-wide">
                    Memories Used
                  </h3>
                  ...
                  <>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Badge
                          variant={
                            embeddingConfig.provider === 'router_cpu'
                              ? 'default'
                              : 'secondary'
                          }
                    ...
                    <div className="text-xs text-stone-500 mt-1">
                      {embeddingConfig.dimensions}dimensions • SIMD: Global{' '}
                      {embeddingConfig.simd_global_enabled !== undefined
                        ? embeddingConfig.simd_global_enabled
                          ? 'Enabled'
                          : 'Disabled'
                        : 'Unknown'}
                      {embeddingConfig.provider === 'router_cpu'
                        ? `, Router ${embeddingConfig.simd_router_enabled !== undefined ? (embeddingConfig.simd_router_enabled ? 'Enabled' : 'Disabled') : 'Unknown'}`
                        : ''}
                    </div>
                  </>
                )}
              ...
              {embeddingConfig && embeddingConfig.provider === 'router_cpu' && (
                <div className="rounded-2xl bg-stone-950/80 border border-zinc-900 px-4 py-3 mb-3">
                  <h4 className="text-stone-100 font-medium text-sm mb-2">
                    Embedding Telemetry
                  </h4>
                  <div className="space-y-1 text-xs">
                    ...
                    <div className="flex justify-between text-stone-400">
                      ...
                      <span className="text-stone-200">
                        {embeddingConfig.simd_router_enabled
                          ? '✅ Enabled (+20-30%)'
                          : '❌ Disabled'}
                      </span>
                    </div>
                    ...
                    {embeddingConfig.performance &&
                      embeddingConfig.cache_ttl_ms !== undefined && (
                        <div className="flex justify-between text-stone-400">
                          <span>Cache TTL:</span>
                          <span className="text-stone-200">
                            {embeddingConfig.cache_ttl_ms / 1000}s
                          </span>
                        </div>
                      )}
                    {streamTelemetry?.stream_duration_ms && (
                      <div className="flex justify-between text-stone-400 mt-2 pt-2 border-t border-stone-800">
                        ...
                        <span className="text-stone-200">
                          {streamTelemetry.stream_duration_ms}ms
                        </span>
                      </div>
                    )}
                  </div>
