...
export function mem(app: any) {
        ...
        try {
            ...
            if (b.user_id) {
                update_user_summary(b.user_id).catch((e) =>
                    logger.error(
                        { component: "MEM", err: e },
                        "[MEM] user summary update failed: %o",
                        e,
                    ),
                );
            }
    ...
    app.post("/memory/ingest", async (req: Request, ctx: Context) => {
            ...
            try {
                ...
                if (text && text.length) {
                    try {
                        ...
                        /* leave as undefined; validation will catch */
                    }
                }
            } catch (e) {
        ...
        try {
            ...
            if (
                ...
                return new Response(
                    JSON.stringify({ err: "file_too_large", msg: e.message }),
                    { status: 413 },
                );
            }
            ...
            return new Response(
                JSON.stringify({ err: "ingest_fail", msg: e.message }),
                { status: 500 },
            );
        }
    });
    ...
    app.post("/memory/ingest/url", async (req: Request, ctx: Context) => {
        ...
        if (!b?.url)
            return new Response(JSON.stringify({ err: "no_url" }), {
                status: 400,
            });
        try {
            ...
            return new Response(
                JSON.stringify({ err: "url_fail", msg: e.message }),
                { status: 500 },
            );
        }
    });
    ...
    app.post("/memory/query", async (req: Request, ctx: Context) => {
        ...
        if (b.metadata) {
            ...
            logger.info(
                {
                    ...
                    batch_mode,
                    router_details: {
                        simd_enabled: router_simd_enabled,
                        fallback_enabled,
                        sector_count: sector_models_summary,
                    },
                },
                "Query telemetry",
            );
            ...
            if (unknownKeys.length > 0) {
                logger.warn(
                    ...
                    "Unknown metadata keys in query telemetry",
                );
            }
        ...
        try {
            ...
            try {
                ...
                logger.warn(
                    { component: "MEM" },
                    "[DEPRECATION] API: /memory/query includes 'matches' and 'memories' (memories is deprecated). Please migrate clients to use 'matches'.",
                );
            } catch (e) {}
            ...
            const accept =
                (req.headers.get ? req.headers.get("accept") : "") || "";
            logger.debug(
                {
                    component: "MEM",
                    accept,
                    testMode: process.env.OM_TEST_MODE,
                },
                "Memory /query accept header",
            );
            ...
            if (
                (accept && accept.includes("text/event-stream")) ||
            ...
            return new Response(
                JSON.stringify({ query: b.query, matches: [] }),
                { headers: { "Content-Type": "application/json" } },
            );
        }
    });
    ...
    app.post("/memory/reinforce", async (req: Request, ctx: Context) => {
        ...
        if (!b?.id)
            return new Response(JSON.stringify({ err: "id" }), { status: 400 });
        try {
    ...
    app.patch("/memory/:id", async (req: Request, ctx: Context) => {
        ...
        try {
            ...
            if (!m)
                return new Response(JSON.stringify({ err: "nf" }), {
                    status: 404,
                });

            if (b.user_id && m.user_id !== b.user_id) {
                return new Response(JSON.stringify({ err: "forbidden" }), {
                    status: 403,
                });
            }
    ...
    app.get("/memory/all", async (req: Request, ctx: Context) => {
        try {
            ...
            logger.warn(
                { component: "MEM" },
                "[DEPRECATION] API: /memory/all includes 'items' and 'memories' (memories is deprecated). Please migrate clients to use 'items'.",
            );
            return new Response(JSON.stringify(out));
        } catch (e: any) {
            if (
                e.message &&
                e.message.includes("user_id when OM_STRICT_TENANT=true")
            ) {
                return new Response(
                    JSON.stringify({
    ...
    app.get("/memory/:id", async (req: Request, ctx: Context) => {
        try {
            ...
            const v = await q.get_vecs_by_id.all(
                id,
    ...
    app.delete("/memory/:id", async (req: Request, ctx: Context) => {
        try {
            const id = ctx.params.id;
            const user_id =
                ctx.query.get("user_id") || (ctx.body as any)?.user_id;
            const m = await q.get_mem.get(id, user_id ?? null);
            if (!m)
                return new Response(JSON.stringify({ err: "nf" }), {
                    status: 404,
