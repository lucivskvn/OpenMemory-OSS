...
describe('Ollama management seam', () => {
    it('returns mocked health and list when __TEST_ollama is set', async () => {
        ...
        embedMod.__TEST_ollama.health = { available: true, version: 'test-version', models_loaded: 2 };
        embedMod.__TEST_ollama.tags = { models: [{ name: 'm1', size: 123 }, { name: 'm2', size: 456 }] };
        ...
        if (typeof mod.startServer === 'function') await mod.startServer({ port: 0, waitUntilReady: true });
