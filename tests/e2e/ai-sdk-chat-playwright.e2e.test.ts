...
async function startDashboard(): Promise<void> {
    ...
    dashboardProcess = spawn('bun', ['run', 'start', `-p`, CONFIG.dashboardPort.toString()], {
        cwd: process.cwd() + '/dashboard',
        stdio: ['ignore', 'pipe', 'pipe'],
        detached: true,
        env: {
...
async function waitForServer(baseUrl: string, endpoint: string, timeout: number): Promise<void> {
    ...
    throw new Error(`Server not ready at ${baseUrl}${endpoint} within ${timeout}ms`)
}
...
describe('AI SDK Chat E2E - Browser Automation', () => {
    ...
    test('displays streaming response and memory content', async () => {
        ...
        const responseContent = page.locator('text=/Based on your stored knowledge/i')
    ...
    test('updates memory sidebar with query results', async () => {
        ...
        const memoryTitles = ['Machine Learning', 'Neural Networks', 'ML Algorithms']
    ...
    test('clears input after successful submission', async () => {
        ...
        // Wait for response
        await expect(page.locator('text=/Based on your stored knowledge/i')).toBeVisible({ timeout: 15000 })
    ...
    test('maintains conversation history', async () => {
        ...
        await sendButton.click()
        await expect(page.locator('text=First question about AI')).toBeVisible({ timeout: 15000 })
        ...
        await expect(page.locator('text=Second question about memories')).toBeVisible()
        await expect(page.locator('text=/Based on your stored knowledge/i')).toHaveCount(2)
    })
    ...
    test('parses telemetry data correctly', async () => {
        ...
        await expect(page.locator('text=/- Retrieved.*Confidence: high/i')).toBeVisible({ timeout: 15000 })
...
describe('Memory Management E2E', () => {
    test('displays memory add-to-bag functionality', async () => {
        ...
        await expect(page.locator('text=/Machine Learning/i')).toBeVisible({ timeout: 15000 })

        // Check for add-to-bag buttons (may use SVG icons or buttons)
        const addButtons = page.locator('button[title*="Add to bag"]').or(page.locator('svg'))
...
describe('Performance & Streaming Benchmarks', () => {
    test('measures end-to-end performance metrics', async () => {
        ...
        // Time to completion
        await expect(page.locator('text=/- Retrieved.*Confidence: high/i')).toBeVisible({ timeout: 10000 })
  ...
  Total response time: ${totalTime}ms`)
    ...
    test('measures streaming throughput', async () => {
        ...
        const startTime = Date.now()
        await expect(page.locator('text=/Memory about AI technology/i').or(
            page.locator('text=/machine learning algorithms/i')
...
describe('Error Recovery & Edge Cases', () => {
    test('handles server unavailability gracefully', async () => {
        ...
        try {
            await expect(page.locator('text=/Based on your stored knowledge/i').or(
