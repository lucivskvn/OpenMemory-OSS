...
**Vulnerability:** In `packages/openmemory-py/src/openmemory/ai/mcp.py`, the MCP tool handlers `openmemory_query`, `openmemory_store`, and `openmemory_list` extracted `user_id` directly from `args.get("user_id")` without calling `_resolve_mcp_tenant(mem, args)`. This allowed any caller to query, store, or list memories and temporal facts belonging to other tenants or bypass tenant scoping entirely on unbound MCP server instances.
**Learning:** When adding helper functions (like `_resolve_mcp_tenant` or `_get_verified_memory`) to validate session credentials across an adapter interface, ensure that every single tool call route consistently delegates to those helpers before executing queries or mutations.
**Prevention:** Always enforce session-based tenant resolution (`_resolve_mcp_tenant`) uniformly across all tool call handlers in protocol adapters like MCP, and reject mismatched caller-supplied `user_id` arguments.
