...
export function embed(app: any): void {
    ...
    app.post("/embed/ollama/pull", async (req: Request, ctx: any) => {
        try {
            ...
            let body: any =
                ctx && (ctx as any).body ? (ctx as any).body : undefined;
            if (!body) {
                try {
            ...
            try {
                ...
                logger.info(
                    { component: "EMBED", modelName, mcp_task_id },
                ...
                if ((__TEST_ollama as any).pull) {
                    const simulated =
                        typeof (__TEST_ollama as any).pull === "function"
                            ? await (__TEST_ollama as any).pull(modelName)
                            : (__TEST_ollama as any).pull;
                ...
                const isCompleted =
                    result.done === true ||
                    (typeof result.done === "string" &&
                        result.done.toLowerCase() === "true") ||
                    pullStatus === "success" ||
                ...
                logger.warn(
                    ...
                    "Ollama unreachable, fallback to synthetic",
                );
                return new Response(
                    JSON.stringify({
                        status: "unavailable",
    ...
    app.get("/embed/ollama/list", async () => {
        try {
            ...
            if ((__TEST_ollama as any).tags !== undefined) {
                const tags =
                    typeof (__TEST_ollama as any).tags === "function"
                        ? await (__TEST_ollama as any).tags()
                        : (__TEST_ollama as any).tags;
                ...
                logger.info(
                    { component: "EMBED", count: models.length },
                    "Retrieved Ollama model list (test seam)",
                );
                return new Response(
        ...
        try {
            ...
            if (
                ...
                logger.warn(
                    ...
                    "Deleting active model - embedding may fail",
                );
            }
            ...
            try {
                ...
                if ((__TEST_ollama as any).delete) {
                    const out =
                        typeof (__TEST_ollama as any).delete === "function"
                            ? await (__TEST_ollama as any).delete(model)
                            : (__TEST_ollama as any).delete;
                ...
                if (response.status === 404 || response.ok) {
                    logger.info(
                        { component: "EMBED", model },
                        "Model deleted (or already absent)",
                    );
                ...
                logger.warn(
                    { component: "EMBED", model, error: String(error) },
                    "Ollama unreachable for delete",
                );
                return new Response(
                    JSON.stringify({
            ...
            try {
                ...
                if (health.available) {
                    logger.info(
                        {
                            component: "EMBED",
                            models_count: health.models_loaded || 0,
                        },
    ...
    app.get("/embed/config", async (req: Request) => {
        try {
            ...
            logger.info(
                { component: "EMBED", provider: result.kind, detailed },
                "Embedding configuration retrieved",
            );
    ...
    app.post("/embed/config", async (req: Request, ctx: any) => {
        try {
            ...
            if (
                ...
                return new Response(
                    JSON.stringify({
                        ...
                        error_code: "invalid_request",
                        message:
                            'Specify at least one of: provider (openai, gemini, ollama, local, router_cpu, synthetic), embed_mode ("simple", "advanced"), or global_simd_enabled (boolean)',
                        context: {
                            timestamp: new Date().toISOString(),
                            path: "/embed/config",
                        },
            ...
            if (
                effectiveMode !== undefined &&
                !["simple", "advanced"].includes(effectiveMode)
            ) {
                return new Response(
                    JSON.stringify({
            ...
            if (provider !== undefined) {
                ...
                if (provider === "router_cpu") {
                    ...
                    runtimeUpdates.router_fallback_enabled =
                        router_fallback_enabled;
                }
            }
            ...
            if (provider === undefined) {
                ...
                if (Object.keys(safeUpdates).length > 0)
                    updateRuntimeConfig(safeUpdates);
            }
            ...
            if (provider !== undefined) {
                ...
                if (provider === "router_cpu") {
                    ...
                    updatesApplied.router_fallback_enabled =
                        router_fallback_enabled;
                }
            }
            ...
            return new Response(
                JSON.stringify({
                    ...
                    context: {
                        distinction_note:
                            'Note: embed_kind selects provider (e.g., router_cpu); embed_mode controls batching (simple/advanced) via OM_EMBED_MODE env var. Use "provider" field for future compatibility.',
                        timestamp: new Date().toISOString(),
                        requested_provider: provider,
                        requested_embed_mode: effectiveMode,
                        current_provider:
                            provider ?? embeddingInfoAfter.provider,
                        current_embed_mode:
                            effectiveMode ?? embeddingInfoAfter.mode,
                    },
                }),
                {
                    status: 200,
