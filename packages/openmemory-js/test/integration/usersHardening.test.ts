...
// Mocks MUST be defined before other imports to take effect
...
import { q, closeDb, runAsync, TABLES } from "../../src/core/db";
...
const DB_PATH = getUniqueDbPath("users_hard");

async function getClientId(apiKey: string): Promise<string> {
    // Must match auth.ts getClientId: returns full 64-char hex hash
...
describe("User Routes Hardening & RBAC", () => {
    ...
    beforeEach(async () => {
        await runAsync(`DELETE FROM ${TABLES.users}`);
        await runAsync(`DELETE FROM ${TABLES.memories}`);
        ...
        // Add 'User' (Owner) Key to DB so they are a valid authenticated user
        const userHashRaw = await crypto.subtle.digest("SHA-256", Buffer.from(userKey));
        const userHash = Buffer.from(userHashRaw).toString("hex");
        await q.insApiKey.run(userHash, userId, "user", "Owner User Key", Date.now(), Date.now(), 0);
