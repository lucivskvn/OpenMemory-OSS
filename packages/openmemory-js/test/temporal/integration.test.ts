import { expect, describe, beforeAll, beforeEach, afterAll } from "bun:test";
...
import { normalizeUserId, sleep } from "../../src/utils";
...
describe("Temporal Graph Integration Suite", () => {
    ...
    const DB_PATH = getUniqueDbPath("temporal");
    ...
    describe("Concurrency & Load", () => {
        ...
        test("Stress Test: High-concurrency temporal insertions", async () => {
            ...
            // Verify that we have history for the overwritten facts
            // We inserted 'count' times. 1 is active. 'count - 1' should be historical.
            // Note: Since they all happen at nearly the same time, we need to ensure the query captures them.
            // getFactsBySubject with ONLY_ACTIVE=false (default is true usually, here 3rd arg is 'onlyActive')
            // Wait, standard getFactsBySubject signature: (subject, predicate, onlyActive, userId)

            // Re-query ALL facts for this subject including history
            const allFacts = await t_query.getFactsBySubject(subject, undefined, true, user1);
            // We expect 'count' total facts (1 active + 49 historical)
            expect(allFacts.length).toBe(count);
        });
    });
});
