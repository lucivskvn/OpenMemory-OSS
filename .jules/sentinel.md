...
## 2026-08-04 - [Missing Tenant Validation and Parameter Resolution in MCP Reinforce Tool]
**Vulnerability:** The `openmemory_reinforce` tool in `packages/openmemory-js/src/ai/mcp.ts` was missing the `user_id` parameter in its Zod schema and did not use `resolve_user_id(tenant, user_id)` like all other MCP tools. Consequently, provided `user_id` parameters were not checked against the authenticated tenant, bypassing `tenant_mismatch` validation.
**Learning:** When creating multi-tool API handlers (such as MCP tool registries), subtle omissions of validation schemas or resolution helpers in individual tools create authorization gaps where tenant verification is bypassed or ignored.
**Prevention:** Standardize tool schemas and authorization resolution across all tools in protocol wrappers (such as MCP), ensuring every tool accepts `user_id` and passes it through `resolve_user_id(tenant, user_id)` before accessing database records.
