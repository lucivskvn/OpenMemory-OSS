...
async function main() {
    ...
    const fixturesDirRoot = path.join(
        __dirname,
        "..",
        "..",
        "tests",
    ...
    const sampleMultiPathBackend = path.join(
        fixturesDirBackend,
        "sample_multi_language.pdf",
    );
    const samplePathRoot = path.join(fixturesDirRoot, "sample.pdf");
    const sampleMultiPathRoot = path.join(
        fixturesDirRoot,
        "sample_multi_language.pdf",
    );
    const sampleScannedPathBackend = path.join(
        fixturesDirBackend,
        "sample_scanned.pdf",
    );
    const sampleLargePathBackend = path.join(
        fixturesDirBackend,
        "sample_large.pdf",
    );
    const sampleScannedPathRoot = path.join(
        fixturesDirRoot,
        "sample_scanned.pdf",
    );
    ...
    await writePdf(samplePathBackend, [
        ...
        {
            options: { addPage: true },
            text: "This is page 2. Hello, world!",
            fontSize: 12,
        },
    ]);
