...
afterEach(async () => {
  ...
  try {
    ...
    try {
      if (__TEST && typeof __TEST.waitForIdle === 'function')
...
test('embeds sectors with user_id present and logs user context', async () => {
  ...
  try {
    ...
    try {
      ...
      if (embedMod && embedMod.__TEST)
        embedMod.__TEST.logHook = (level: any, meta: any) => {
          try {
            if (meta && meta.user_id) seen = meta.user_id;
          } catch (e) {}
        };
    ...
    /* ignore, spyLoggerMethod is tolerant */
  }

  const embedMod: any = await import('../../backend/src/memory/embed');
  const res = await embedMod.embedMultiSector(
    'e1',
    'Hello OpenMemory',
    ['semantic', 'episodic'],
    undefined,
  ...
  if (!seen) {
    ...
    expect(
      pending.find((r: any) => r.id === 'e1' && r.user_id === 'user1'),
    ).toBeTruthy();
  } else {
  ...
  // restore logger
  try {
    if (handleInfo && typeof handleInfo.restore === 'function')
      handleInfo.restore();
  } catch (e) {}
  try {
    const embedMod: any = await import('../../backend/src/memory/embed');
...
describe('embedLog threshold enforcement', () => {
  ...
  it('suppresses debug/info logs when OM_LOG_EMBED_LEVEL=warn', async () => {
    ...
    // spy.
    const { spyLoggerMethod } = await import('../utils/spyLoggerSafely');
    let handleInfo2: any = spyLoggerMethod(
      loggerMod,
      'info',
      (meta: any, msg?: any) => {
        calls.push({ level: 'info', meta, msg });
      },
    );
    let handleDebug2: any = spyLoggerMethod(
      loggerMod,
      'debug',
      (meta: any, msg?: any) => {
        calls.push({ level: 'debug', meta, msg });
      },
    );
    ...
    if (typeof embedMod.__setTestProvider === 'function') {
      embedMod.__setTestProvider(async (t: string, s: string) =>
        embedMod.gen_syn_emb(t, s),
      );
    }
    ...
    try {
      ...
      const embedCalls = calls.filter((c) => {
        try {
          return (
            (c.meta && c.meta.component === 'EMBED') ||
            (typeof c.msg === 'string' && c.msg.includes('[EMBED]'))
          );
        } catch (e) {
          return false;
        }
      ...
      try {
        if (handleInfo2 && typeof handleInfo2.restore === 'function')
          handleInfo2.restore();
      } catch (_) {}
      try {
        if (handleDebug2 && typeof handleDebug2.restore === 'function')
          handleDebug2.restore();
      } catch (_) {}
      try {
        if (origWarn) loggerMod.default.warn = origWarn;
      } catch (_) {}
  ...
  it('emits debug/info logs when OM_LOG_EMBED_LEVEL=debug', async () => {
    ...
    let handleInfo3: any = spy3(loggerMod, 'info', (meta: any, msg?: any) => {
      calls.push({ level: 'info', meta, msg });
    });
    let handleDebug3: any = spy3(loggerMod, 'debug', (meta: any, msg?: any) => {
      calls.push({ level: 'debug', meta, msg });
    });
    // Also attach an embed test hook so we capture embed logs reliably
    try {
    ...
    try {
      ...
      if (embedCalls.length > 0) {
        ...
        const withUser = embedCalls.find(
          (c) => c.meta && c.meta.user_id === 'test-user-debug',
        );
        expect(withUser).toBeTruthy();
      }
    } finally {
      try {
        if (handleInfo3 && typeof handleInfo3.restore === 'function')
          handleInfo3.restore();
      } catch (_) {}
      try {
        const embedMod: any = await import('../../backend/src/memory/embed');
      ...
      try {
        if (handleDebug3 && typeof handleDebug3.restore === 'function')
          handleDebug3.restore();
      } catch (_) {}
      delete process.env.OM_LOG_EMBED_LEVEL;
    }
  });
...
test('provider failure triggers gemini fallback to synthetic', async () => {
  ...
  try {
    ...
    const dim =
      embedMod.getEmbeddingInfo().dimensions || Number(process.env.OM_VEC_DIM);
    expect(out[0].vector.length).toBe(dim);
  } finally {
    (globalThis as any).fetch = origFetch;
    if (_oldVec === undefined) delete process.env.OM_VEC_DIM;
    else process.env.OM_VEC_DIM = _oldVec;
  }
});
