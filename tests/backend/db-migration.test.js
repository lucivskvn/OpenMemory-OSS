...
describe('DB migration & multi-tenant verification', () => {
  ...
  it('enforces OM_STRICT_TENANT on reads', async () => {
    ...
    try {
      ...
      expect(
        msg.includes('tenant-scoped') || msg.includes('requires user_id'),
      ).toBe(true);
    }
  ...
  it('sets SQLite PRAGMA journal_mode to WAL on init', async () => {
    ...
    const val =
      typeof row === 'string'
        ? row
        : row?.journal_mode || (row && Object.values(row)[0]);
...
(process.env.TEST_POSTGRES_URL ? describe : describe.skip)(
  'Postgres strict-tenant enforcement',
  () => {
    beforeEach(async () => {
