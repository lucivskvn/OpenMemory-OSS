...
describe('getEmbeddingConfig success and cache', () => {
    test('fetch returns parsed config and caching works', async () => {
        ...
        globalThis.fetch = (async (url: string) => ({ ok: true, json: async () => ({ status: 'configuration_updated', new_provider: 'openai', previous_provider: 'synthetic' }) }) as any) as any
...
describe('updateEmbeddingProvider success', () => {
    test('updateEmbeddingProvider returns metadata and clears cache', async () => {
        ...
        globalThis.fetch = (async (url: string) => {
            callCount++
            return { ok: true, json: async () => ({ status: 'configuration_updated', new_provider: 'openai', previous_provider: 'synthetic', message: 'updated' }) } as any
        }) as any
