...
- Improve `20240108000000_pgvector_finalize.sql`: added typmod/dimension checks and backfill advisories to guide safe migration and index creation; added batch-safe `tools/backfill_pgvector.ts` with dimension validation (`OM_VEC_DIM`) and `BACKFILL_BATCH` support.
...
- Add dual-layer rate limiting (IP-based DoS protection + per-key quotas) in `authPlugin` and tests to prevent brute-force and quota abuse.
