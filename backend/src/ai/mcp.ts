...
export const create_mcp_srv = () => {
    const srv = new McpServer(
        { name: "openmemory-mcp", version: "2.1.0" },
    ...
    srv.tool(
        ...
        { title: "Run a semantic retrieval against OpenMemory" },
    ...
    srv.tool(
        ...
        async ({ id, include_vectors, user_id }) => {
            ...
            const vecs = include_vectors
                ? await q.get_vecs_by_id.all(id, mem.user_id ?? null)
                : [];
            const pay = {
                id: mem.id,
                content: mem.content,
    ...
    srv.resource(
        ...
        async () => {
            ...
            const pay = {
                ...
                stats,
                embeddings: await getEmbeddingInfo(),
