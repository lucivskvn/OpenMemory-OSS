...
function generate(outputPath) {
    ...
    for (let p = 1; p <= 3; p++) {
        ...
        const margin = 40;
        doc.fill('#dcdcdc').rect(margin, margin, doc.page.width - margin * 2, doc.page.height - margin * 2).fill();

        // Stamp some faint lines to simulate scanner artifacts
        doc.fill('#cfcfcf');
        for (let y = margin + 30; y < doc.page.height - margin - 30; y += 40) {
            doc.rect(margin + 10, y, doc.page.width - margin * 2 - 20, 1).fill();
        }

        // Overlay a small "scanned" label
        doc.fill('#666').fontSize(10).text(`Scanned Page ${p}`, margin + 12, margin + 12);
    }
...
if (require.main === module) {
    const outPath = path.join(__dirname, '..', '..', 'tests', 'fixtures', 'sample_scanned.pdf');
    generate(outPath)
        .then(() => console.log('Generated scanned-like sample PDF at', outPath))
        .catch((e) => {
