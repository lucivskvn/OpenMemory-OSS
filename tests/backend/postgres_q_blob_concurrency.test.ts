...
test(
  'postgres q blob/array/concurrency: binary bindings, array+scalar mix, high-concurrency reuse',
  async () => {
    ...
    try {
      ...
      const gotBlob = await get(
        `select compressed_vec from ${memTable} where id=$1`,
        [idBlob],
      );
      ...
      const rows = await all(
        `select id from ${memTable} where tags::jsonb @> $1::jsonb and user_id = $2`,
        [JSON.stringify(['alpha']), 'pg-arr'],
      );
      ...
      await Promise.all(
        ids.map((iid) =>
          run(
            ...
            [iid, 'pg-conc', 'concurrent', 'pg-conc', Date.now(), Date.now()],
          ),
        ),
      );

      const found = await all(
        `select id from ${memTable} where user_id = $1 and content = $2`,
        ['pg-conc', 'concurrent'],
      );
      ...
      await run(`delete from ${memTable} where primary_sector in ($1,$2,$3)`, [
        'pg-blob-parity',
        'pg-adv',
        'pg-conc',
      ]);
      await run(`delete from ${memTable} where id in ($1,$2,$3)`, [
        idBlob,
        idArray,
        ids[0],
      ]);
