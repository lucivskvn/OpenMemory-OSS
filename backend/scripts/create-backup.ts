...
async function createBackup() {
    try {
        ...
        await backupDatabase({
            ...
            progressCallback: (progress) => {
                process.stdout.write(
                    `\rProgress: ${progress.percentage}% (${progress.totalPages - progress.remainingPages}/${progress.totalPages})`,
                );
            },
        });
        ...
        backups.slice(0, 5).forEach((backup) => {
            ...
            console.log(
                `  ${backup.filename} (${age}h ago, ${(backup.size / 1024 / 1024).toFixed(1)}MB)`,
            );
        });
    } catch (error) {
