  ...
  async function fetchdata() {
    ...
    try {
      const [statsres, memsres] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/memory/all?l=100&u=0`, {
          headers: getHeaders(),
        }),
      ]);
      ...
      const sectorstats = sectors.map((sector) => {
        const sectormems = (memsdata.items || []).filter(
          (m: any) => m.primary_sector === sector,
        );
        ...
        const avgdecay =
          sectormems.length > 0
            ? sectormems.reduce(
                (sum: number, m: any) => sum + (m.decay_lambda || 0.01),
                0,
              ) / sectormems.length
            : 0.01;
        const avgSalience =
          sectormems.length > 0
            ? sectormems.reduce((sum: number, m: any) => sum + m.salience, 0) /
              sectormems.length
            : 1.0;
        ...
        <div className="space-y-6" suppressHydrationWarning>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {stats.map((s) => (
              <div
                key={s.sector}
                className="rounded-xl border border-stone-900 bg-stone-950 p-4"
              >
                <h3 className="text-sm font-medium text-stone-400 mb-3 capitalize">
                  {s.sector}
                </h3>
                <div className="space-y-3">
                  <div>
                    ...
                    <p
                      className={`text-2xl font-bold ${getdecaycolor(s.avgdecay)}`}
                    >
            ...
            <div className="space-y-3" suppressHydrationWarning>
              {riskmems.length === 0 ? (
                ...
                riskmems.map((mem) => (
                  <div
                    key={mem.id}
                    className="rounded-xl border border-rose-500/15 bg-rose-500/10 p-4 hover:bg-rose-500/15 transition-colors"
                    suppressHydrationWarning
                  >
                    <div
                      ...
                      <div className="flex-1" suppressHydrationWarning>
                        ...
                        >
                          ...
                          <span className="text-xs text-rose-400">
                            Salience: {mem.salience.toFixed(2)}
                          </span>
                        </div>
