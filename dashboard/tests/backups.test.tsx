...
if (typeof document !== 'undefined' && typeof window !== 'undefined') {
    describe('BackupsPage Component - React RTL Tests', () => {
        test('renders BackupsPage component with correct UI elements', async () => {
            ...
            server.use(
                rest.get('http://localhost:8080/admin/backup/status', async (req, res, ctx) => {
                    return res(ctx.json({
                ...
                rest.get('http://localhost:8080/admin/backup/list', async (req, res, ctx) => {
                    return res(ctx.json({
                        backups: [
                            {
        ...
        test('displays backup progress with SSE when backup starts', async () => {
            ...
            server.use(
                ...
                rest.post('http://localhost:8080/admin/backup', async (req, res, ctx) => {
                    return res(ctx.json({
                        success: true,
                ...
                rest.get('http://localhost:8080/admin/backup/progress/:sessionId', async (req, res, ctx) => {
                    ...
                    const response = progressStates[progressCallCount++ % progressStates.length]
        ...
        test('shows integrity check alert on successful restore', async () => {
            ...
            server.use(
                ...
                rest.post('http://localhost:8080/admin/backup/restore', async (req, res, ctx) => {
                    return res(ctx.json({
                        success: true,
        ...
        test('refreshes backup list and status correctly', async () => {
            ...
            server.use(
                rest.get('http://localhost:8080/admin/backup/status', async (req, res, ctx) => {
                    statusCallCount++
                    return res(ctx.json({
                        lastBackup: statusCallCount > 1 ? new Date().toISOString() : null,
    ...
    console.warn('Skipping BackupsPage DOM tests: document/window not available in this test environment')
}
