...
test("postgres q advanced: json/array params, prepared reuse, error paths", async () => {
    ...
    await run(
        ...
        [idJson, "pg-adv", "json-test", "pg-q-adv", JSON.stringify(meta), now, now],
    );
    const got = await get(`select meta from ${memTable} where id=$1`, [idJson]);
    // Some Postgres clients return JSON as string, others already parsed; accept both
    const parsed = typeof got.meta === "string" ? JSON.parse(got.meta) : got.meta;
    ...
    await run(
        ...
        [idArr1, "pg-adv", "arr-test", "pg-q-adv", JSON.stringify(arr), now, now],
    );
    const gotArr = await get(`select tags from ${memTable} where id=$1`, [idArr1]);
    const parsedArr = typeof gotArr.tags === "string" ? JSON.parse(gotArr.tags) : gotArr.tags;
    ...
    const reuseIds = [crypto.randomUUID(), crypto.randomUUID(), crypto.randomUUID()];
    for (const rid of reuseIds) {
        await run(`insert into ${memTable}(id,user_id,content,primary_sector,created_at,updated_at) values($1,$2,$3,$4,$5,$6)`, [rid, "pg-adv", "reuse", "pg-q-adv", Date.now(), Date.now()]);
    }
    const found = await all(`select id from ${memTable} where content = $1 order by id`, ["reuse"]);
    ...
    try {
        ...
        expect(e.message || e.toString()).toMatch(/no such table|relation .* does not exist|does not exist/i);
    }
