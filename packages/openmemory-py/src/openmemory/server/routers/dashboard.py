...
async def get_stats(user_id: Optional[str] = None, auth_user: str = Depends(get_current_user_id)):
    ...
    return {
        ...
        "decayStats": {
            "total": count_val,
            "avgLambda": f"{agg['avgl'] or 0.0:.3f}",
            "minSalience": f"{agg['mins'] or 0.0:.2f}",
            "maxSalience": f"{agg['maxs'] or 0.0:.2f}"
        },
        "requests": {
        ...
        "qps": {
            ...
            "average": metrics.total_requests / (time.time() - metrics.start_time) if (time.time() - metrics.start_time) > 0 else 0.0,
