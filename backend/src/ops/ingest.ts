...
const link = async (
    ...
    try {
        ...
        await _transaction.rollback();
        logger.error(
            { component: "INGEST", err: e, section: idx },
            "Link failed",
        );
        throw e;
    }
...
export async function ingestDocument(
    ...
    if ((ingestDocument as any)._mock)
        return await (ingestDocument as any)._mock(t, data, meta, cfg, user_id);
    ...
    if (!useRC) {
        ...
        const _add_hsg_memory =
            (ingestDocument as any)._add_hsg_memory || add_hsg_memory;
    ...
    logger.info(
        { component: "INGEST", tokens: exMeta.estimated_tokens },
        "Document token count",
    );
    logger.info(
        { component: "INGEST", sections: secs.length },
    ...
    try {
        ...
        logger.info(
            { component: "INGEST", root: rid, linked: cids.length },
            "Completed sections linked",
        );
        return {
            root_memory_id: rid,
...
export function setIngestDocumentForTests(
    mock?:
        | ((
              t: string,
              data: string | Buffer,
...
export async function ingestDocumentFromFile(
    ...
    try {
        ...
        const envMax =
            Number(process.env.OM_INGEST_MAX_SIZE_MB || "") || undefined;
        ...
        if (fileSize == null) {
            ...
            const reader = (f as any).stream
                ? (f as any).stream().getReader()
                : null;
            if (reader) {
        ...
        const normProvided = (contentType || "")
            .split(";")[0]
            .trim()
            .toLowerCase();
        const normDetected = (mimeType || "")
            .split(";")[0]
            .trim()
            .toLowerCase();
        if (normProvided && normDetected && normProvided !== normDetected) {
            ...
            logger.warn(
                ...
                "Provided contentType differs from detected mime",
            );
        }
        ...
        const preferDetected =
            cfg?.prefer_detected_mime ||
            (process.env.OM_INGEST_PREFER_DETECTED || "").toLowerCase() ===
                "true";
        if (preferDetected && normProvided.startsWith("text") && normDetected) {
            ...
            if (preferSet.has(normDetected)) {
                ...
                logger.info(
                    ...
                    "Overriding provided contentType with detected MIME since prefer_detected_mime is enabled",
                );
            }
        }
        ...
        if (!effectiveType) {
            ...
            if (p.endsWith(".pdf")) effectiveType = "application/pdf";
                ...
                effectiveType = "text/html";
            else if (p.endsWith(".md") || p.endsWith(".markdown"))
                effectiveType = "text/markdown";
        ...
        if (isMultiModal) {
            const temporalMetadata = extractTemporalMetadata(
                effectiveType,
            ...
            if (temporalMetadata) {
                ...
                if (result.strategy === "single") {
                    ...
                    logger.info(
                        {
                            component: "INGEST",
                            temporalType: temporalMetadata.type,
                        },
                        "Temporal tag attached to multi-modal content",
                    );
                }
            }
        }
        ...
        logger.error(
            { component: "INGEST", file: filePath, err: e },
...
export async function ingestURL(
    ...
    logger.info(
        { component: "INGEST", tokens: ex.metadata.estimated_tokens },
        "URL token count",
    );
    logger.info(
        { component: "INGEST", sections: secs.length },
    ...
    try {
        ...
        logger.info(
            { component: "INGEST", root: rid },
            "Root memory for URL created",
        );
        for (let i = 0; i < secs.length; i++) {
            try {
                const cid = await mkChild(
