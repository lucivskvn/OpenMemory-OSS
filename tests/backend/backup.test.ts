...
import { backupDatabase, exportDatabaseDump, vacuumIntoBackup, restoreFromBackup, listBackups, uploadToSupabaseStorage, downloadFromSupabaseStorage, importDatabaseDump, type BackupOptions, type RestoreOptions } from "../../backend/src/utils/backup.js";
...
describe("SQLite Backup Utilities", () => {
    ...
    describe("backupDatabase", () => {
        it("should create a backup successfully", async () => {
            ...
            const result = backupDb.prepare("SELECT COUNT(*) as count FROM test_memories").get() as { count: number };
        ...
        it("should handle WAL checkpointing", async () => {
            ...
            db.exec("INSERT INTO test_memories VALUES ('3', 'test content 3', 1234567892);");
            db.close();
    ...
    describe("vacuumIntoBackup", () => {
        ...
        it("should reduce file size after vacuum", async () => {
            ...
            const db = new DatabaseSync(testDbPath);
            for (let i = 3; i <= 100; i++) {
                db.exec(`INSERT INTO test_memories VALUES ('${i}', 'content ${i}', ${1234567890 + i});`);
            }
    ...
    describe("restoreFromBackup", () => {
        it("should restore database successfully", async () => {
            ...
            const result = restoredDb.prepare("SELECT COUNT(*) as count FROM test_memories").get() as { count: number };
            expect(result.count).toBe(2);
        ...
        it("should perform clean restore from SQL dump, removing old tables/data not present in dump", async () => {
            ...
            `);
            const extraTableExistsBefore = existingDb.prepare("SELECT COUNT(*) as count FROM sqlite_master WHERE name='extra_table'").get() as { count: number };
            expect(extraTableExistsBefore.count).toBe(1);
            const oldDataCount = existingDb.prepare("SELECT COUNT(*) as count FROM test_memories WHERE id LIKE 'old-%'").get() as { count: number };
            ...
            const extraTableExistsAfter = restoredDb.prepare("SELECT COUNT(*) as count FROM sqlite_master WHERE name='extra_table'").get() as { count: number };
            expect(extraTableExistsAfter.count).toBe(0); // Should be removed

            // Check that old data was removed
            const newDataCount = restoredDb.prepare("SELECT COUNT(*) as count FROM test_memories WHERE id LIKE 'old-%'").get() as { count: number };
            expect(newDataCount.count).toBe(0); // Should be removed

            // Check that new data from dump is present
            const originalDataCount = restoredDb.prepare("SELECT COUNT(*) as count FROM test_memories").get() as { count: number };
            expect(originalDataCount.count).toBe(2); // Should match original dump

            // Verify specific data
            const specificRecord = restoredDb.prepare("SELECT * FROM test_memories WHERE id = '1'").get() as any;
                ...
                send: vi.fn().mockResolvedValue({
                    Body: {
                        transformToWebStream: () => ({
                            getReader: () => ({
                                read: vi.fn()
                                    .mockResolvedValueOnce({ value: new Uint8Array([116, 101, 115, 116]), done: false })
    ...
    describe("importDatabaseDump", () => {
        it("should create database from SQL dump successfully", async () => {
            ...
            const result = importedDb.prepare("SELECT COUNT(*) as count FROM test_memories").get() as { count: number };
            expect(result.count).toBe(2);

            // Verify integrity
            const integrity = importedDb.prepare("PRAGMA integrity_check").get() as { integrity_check: string };
            expect(integrity.integrity_check).toBe("ok");
        ...
        it("should handle non-existent dump file", async () => {
            ...
            await expect(importDatabaseDump("/nonexistent/dump.sql", importedDbPath)).rejects.toThrow();
        });
        ...
        it("should handle invalid SQL in dump", async () => {
            ...
            await expect(importDatabaseDump(invalidDumpPath, importedDbPath)).rejects.toThrow();
        });
        ...
        it("should handle semicolons in string literals properly", async () => {
            ...
            const result = importedComplexDb.prepare("SELECT * FROM complex_test ORDER BY id").all() as { id: number; content: string }[];
...
-- End comment
            ...
            const result = commentDb.prepare("SELECT * FROM test_comments").all() as { id: number; content: string }[];
...
ANALYZE;
            ...
            const pragmaResult = schemaDb.prepare("PRAGMA foreign_keys").get() as { foreign_keys: number };
            expect(pragmaResult.foreign_keys).toBe(1);

            // Check table exists
            const tableResult = schemaDb.prepare("SELECT COUNT(*) as count FROM sqlite_master WHERE type='table' AND name='users'").get() as { count: number };
            expect(tableResult.count).toBe(1);

            // Check index exists
            const indexResult = schemaDb.prepare("SELECT COUNT(*) as count FROM sqlite_master WHERE type='index' AND name='idx_users_name'").get() as { count: number };
            expect(indexResult.count).toBe(1);

            // Check view exists
            const viewResult = schemaDb.prepare("SELECT COUNT(*) as count FROM sqlite_master WHERE type='view' AND name='active_users'").get() as { count: number };
            expect(viewResult.count).toBe(1);

            // Check data exists
            const dataResult = schemaDb.prepare("SELECT COUNT(*) as count FROM users").get() as { count: number };
    ...
    describe("Error handling", () => {
        ...
        it("should handle cloud upload errors", async () => {
            ...
            await expect(uploadToSupabaseStorage("/test/path", "test-key", mockS3Client)).rejects.toThrow("Upload failed");
        });
        ...
        it("should handle cloud download errors", async () => {
            ...
            await expect(downloadFromSupabaseStorage("test-key", "/test/path", mockS3Client)).rejects.toThrow("Download failed");
        });
    });
});
