...
async def add_memory(req: AddRequest, auth_user: str = Depends(get_current_user_id)):
    ...
    return await mem_client.add(req.content, user_id=uid, tags=req.tags, meta=req.metadata)
...
async def ingest_memory(req: IngestRequest, auth_user: str = Depends(get_current_user_id)):
    ...
    res = await ingest_document(req.content_type, req.data, meta=req.metadata, user_id=uid)
    return res
...
async def get_memory(id: str, user_id: Optional[str] = None, auth_user: str = Depends(get_current_user_id)):
    uid = resolve_user(auth_user, user_id)
    item = await mem_client.get(id, user_id=uid)
    if not item:
        raise HTTPException(status_code=404, detail="Memory not found")
    return item

@router.delete("/{id}")
async def delete_memory(id: str, user_id: Optional[str] = None, auth_user: str = Depends(get_current_user_id)):
    try:
        uid = resolve_user(auth_user, user_id)
        await mem_client.delete(id, user_id=uid)
        return {"status": "ok"}
...
async def list_memories(limit: int = 100, offset: int = 0, user_id: Optional[str] = None, auth_user: str = Depends(get_current_user_id)):
    uid = resolve_user(auth_user, user_id)
    items = await mem_client.history(user_id=uid, limit=limit, offset=offset)
    return {"items": items}
...
async def update_memory(id: str, req: Dict[str, Any], auth_user: str = Depends(get_current_user_id)): # Simple dict for patch
    ...
    try:
         item = await mem_client.update(id, content=content, tags=tags, metadata=meta, user_id=uid)
         return item
    except ValueError:
        raise HTTPException(status_code=404, detail="Memory not found")
    except PermissionError:
