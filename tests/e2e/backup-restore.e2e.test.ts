...
describe("Backup and Restore E2E Tests", () => {
    ...
    describe("Backup API Integration", () => {
        test("should successfully trigger a backup via API", async () => {
            const response = await fetch(`http://localhost:${TEST_PORT}/admin/backup`, {
                method: "POST",
                headers: {
        ...
        test("should list available backups", async () => {
            const response = await fetch(`http://localhost:${TEST_PORT}/admin/backup/list`, {
                headers: {
                    "Authorization": `Bearer ${BACKUP_API_KEY}`
                }
            });
        ...
        test("should provide backup status information", async () => {
            const response = await fetch(`http://localhost:${TEST_PORT}/admin/backup/status`, {
                headers: {
                    "Authorization": `Bearer ${BACKUP_API_KEY}`
                }
            });
    ...
    describe("Restore API Integration", () => {
        test("should successfully restore from backup", async () => {
            ...
            const listResponse = await fetch(`http://localhost:${TEST_PORT}/admin/backup/list`, {
                headers: {
                    "Authorization": `Bearer ${BACKUP_API_KEY}`
                }
            });
            ...
            const modifiedCount = modifiedDb.prepare("SELECT COUNT(*) as count FROM memories").get() as { count: number };
            expect(modifiedCount.count).toBe(4); // 3 original + 1 new - 1 deleted
            modifiedDb.close();

            // Restore from backup
            const restoreResponse = await fetch(`http://localhost:${TEST_PORT}/admin/backup/restore`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${BACKUP_API_KEY}`
            ...
            const restoredCount = restoredDb.prepare("SELECT COUNT(*) as count FROM memories").get() as { count: number };
            expect(restoredCount.count).toBe(4); // Should now have original data back

            // Check that the deleted memory is back
            const memory1Exists = restoredDb.prepare("SELECT COUNT(*) as count FROM memories WHERE id = 'mem-1'").get() as { count: number };
            expect(memory1Exists.count).toBe(1);

            // Check that the new memory is gone (since we restored from backup)
            const memory5Exists = restoredDb.prepare("SELECT COUNT(*) as count FROM memories WHERE id = 'mem-5'").get() as { count: number };
    ...
    describe("Error Handling", () => {
        ...
        test("should handle missing backup file gracefully", async () => {
            const response = await fetch(`http://localhost:${TEST_PORT}/admin/backup/restore`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${BACKUP_API_KEY}`
    ...
    describe("WAL Mode Handling", () => {
        test("should handle WAL checkpoints during backup", async () => {
            ...
            db.exec("INSERT INTO memories VALUES ('mem-wal-1', 'user-1', 'WAL test memory', 1234567895000)");
            db.close();
            ...
            // Perform backup
            const backupResponse = await fetch(`http://localhost:${TEST_PORT}/admin/backup`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${BACKUP_API_KEY}`
            ...
            const walMemory = backupDb.prepare("SELECT COUNT(*) as count FROM memories WHERE id = 'mem-wal-1'").get() as { count: number };
