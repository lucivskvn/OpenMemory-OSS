...
start: ## Start production server
	...
	cd backend && bun start || (cd backend && bun run start)

stop: ## Stop server (if running as daemon)
	@echo "🛑 Stopping server..."
	@pgrep -f 'openmemory' | xargs -r kill || echo "No server process found"

# Testing
test: ## Run all tests
	@echo "🧪 Running all tests..."
	@echo "Testing backend API..."
	cd backend && bun test ../tests/backend/
	@echo "Testing JavaScript SDK..."
	bun tests/js-sdk/sdk-simple.test.js
...
lint: ## Run linters
	...
	cd backend && (bun run lint || echo "No backend lint script or lint passed")
...
type-check: ## Run type checking
	...
	cd sdk-js && bun run typecheck || bun tsc --noEmit
...
run-examples: ## Run example files
	...
	bun examples/backend/basic-server.js &
	sleep 2
	bun examples/backend/api-test.mjs
