...
async function validate_api_key(
    provided: string,
    hashed_expected: string,
): Promise<boolean> {
    ...
    try {
        ...
        if (
            isHashedKey(hashed_expected) ||
            hashed_expected.startsWith("argon2") ||
            hashed_expected.startsWith("$argon2")
        ) {
            const ok = await verifyPassword(provided, hashed_expected);
            if (!ok)
                logger.warn(
                    { component: "AUTH", event: "auth_failure" },
        ...
        logger.error(
            { component: "AUTH", event: "plaintext_key_detected" },
            "Plaintext API key detected in configuration; plaintext API keys are not accepted. Please set OM_API_KEY to a hashed value (use backend/scripts/hash-api-key.ts) and restart.",
        );
        return false;
    } catch (e) {
        logger.warn(
            { component: "AUTH", event: "verify_error", err: e },
            "Password verify failed for API key; rejecting authentication",
        );
        return false;
    }
}
...
function get_client_id(req: Request, api_key: string | null): string {
    if (api_key) return hashString(api_key).slice(0, 16);
 ...
 */
export async function authenticate_api_request(
    req: Request,
    ctx: Context,
    ...
    logger.debug(
        { component: "AUTH", method: req.method, path: url.pathname },
        "Auth check method/path",
    );
    ...
    if (!rl.allowed) {
        ...
        if (auth_config.rate_limit_enabled) {
            headers.set(
                "X-RateLimit-Limit",
                auth_config.rate_limit_max_requests.toString(),
            );
            headers.set("X-RateLimit-Remaining", rl.remaining.toString());
            headers.set(
                "X-RateLimit-Reset",
                Math.floor(rl.reset_time / 1000).toString(),
            );
            headers.set(
                "Retry-After",
                Math.ceil((rl.reset_time - Date.now()) / 1000).toString(),
            );
        }
        return new Response(
            JSON.stringify({
    ...
    if (auth_config.rate_limit_enabled) {
        try {
            ...
            if ((response as any).bodyUsed || !response.body) {
                ...
                logger.debug(
                    { component: "AUTH" },
                    "Response body already used or null; skipping header merge to preserve streaming behavior",
                );
                return response;
            }
            ...
            logger.warn(
                { component: "AUTH", err: e },
                "Failed to attach rate-limit headers by constructing a new Response; returning original response",
            );
            return response;
        }
    }
...
export function log_authenticated_request(
    req: Request,
    ctx: Context,
    next: () => Promise<Response>,
) {
...
export async function adminAuthMiddleware(
    ...
    const url = new URL(req.url);
    logger.debug(
        { component: "AUTH-ADMIN", method: req.method, path: url.pathname },
        "Admin auth check",
    );
    ...
    if (!env.admin_api_key || env.admin_api_key === "") {
        logger.warn(
            ...
            "No admin API key configured, denying all admin requests.",
        );
    ...
    if (env.rate_limit_enabled) {
        try {
            if ((response as any).bodyUsed || !response.body) {
                logger.debug(
                    { component: "AUTH-ADMIN" },
                    "Response body already used or null; skipping header merge",
                );
                return response;
            }
            const merged = new Headers(response.headers);
            ...
            logger.warn(
                { component: "AUTH-ADMIN", err: e },
                "Failed to attach rate-limit headers",
            );
            return response;
        }
    }
