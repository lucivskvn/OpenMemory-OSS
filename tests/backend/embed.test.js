...
afterEach(() => {
  if (_oldOm === undefined) delete process.env.OM_LOG_LEVEL;
  else process.env.OM_LOG_LEVEL = _oldOm;
  if (_oldEmbed === undefined) delete process.env.OM_LOG_EMBED_LEVEL;
  else process.env.OM_LOG_EMBED_LEVEL = _oldEmbed;
...
beforeEach(async () => {
  ...
  try {
    if (
      embedMod &&
      embedMod.__TEST &&
      typeof embedMod.__TEST.reset === 'function'
    )
  ...
  try {
    const dim =
      (process.env.OM_VEC_DIM && parseInt(process.env.OM_VEC_DIM)) || 128;
    const defaultProvider = async () =>
      Array.from({ length: dim }, (_, i) => (i + 1) / dim);
    const defaultBatchProvider = async (texts) =>
  ...
  try {
    ...
    try {
      ...
      if (
        embed &&
        embed.__TEST &&
        typeof embed.__TEST.waitForIdle === 'function'
      ) {
        try {
          await embed.__TEST.waitForIdle();
        } catch (e) {}
      }
    } catch (e) {}
    await closeDb();
...
describe('Embedding layer integration tests', () => {
  afterEach(async () => {
    ...
    try {
      if (
        embedMod &&
        embedMod.__TEST &&
        typeof embedMod.__TEST.waitForIdle === 'function'
      ) {
        try {
  ...
  it('local embedding returns normalized vector and records logs', async () => {
    ...
    const logRow = await get_async('select status from embed_logs where id=?', [
      id,
    ]);
  ...
  describe('failure and retry behavior', () => {
    ...
    beforeEach(() => {
      ...
      try {
        origTestProvider = embedMod.__TEST && embedMod.__TEST.provider;
      } catch (e) {
        origTestProvider = null;
      }
    });
    ...
    afterEach(() => {
      ...
      try {
        if (
          ...
          embedMod.__TEST.reset();
        else if (embedMod && typeof embedMod.__setTestProvider === 'function')
          embedMod.__setTestProvider(origTestProvider);
      } catch (e) {}
    });
    ...
    it('marks embed_logs failed after retries and logs error with user_id', async () => {
      ...
      try {
        if (typeof embedMod.__setTestProvider === 'function') {
          embedMod.__setTestProvider(async () => {
            calls++;
            throw new Error('Simulated API failure');
          });
        } else if (embedMod.__TEST) {
          embedMod.__TEST.provider = async () => {
            calls++;
            throw new Error('Simulated API failure');
          };
        }
      ...
      try {
        ...
        handle = spyLoggerMethod(loggerMod, 'error', (...args) => {
          errors.push(args);
        });
      } catch (e) {}
      ...
      let threw = false;
      try {
        await embedMod.embedMultiSector(
          id,
          'will fail',
          ['semantic'],
          undefined,
      ...
      const row = await get_async(
        'select status,err from embed_logs where id=?',
        [id],
      );
      ...
      const found = errors.find((args) =>
        args.some(
          (a) =>
            (String(a).includes && String(a).includes('multi-sector failed')) ||
            String(a).includes('multi-sector failed'),
        ),
      );
      ...
      if (found) {
        ...
        const userArg = found.find(
          (a) => a && typeof a === 'object' && a.user_id,
        );
