...
function decayLog(
    ...
    try {
        ...
        try {
            console.error("[DECAY] log failure", e);
        } catch (_err) {}
    }
}
...
export const apply_decay = async () => {
    if (active_q > 0) {
        decayLog(
            "info",
            { component: "DECAY" },
            `[DECAY] skipped - ${active_q} active queries`,
        );
        return;
    }
            ...
            parts.map(async (part) => {
                for (const m of part) {
                    ...
                    if (f < 0.7) {
                        ...
                        const vec_row = await q.get_vec.get(
                            m.id,
                            sector,
                            m.user_id ?? null,
                        );
...
export const on_query_hit = async (
    ...
    if (updated) {
        decayLog(
            "info",
            { component: "DECAY" },
            `[DECAY-2.0] regenerated/reinforced memory ${mem_id}`,
        );
    }
};
