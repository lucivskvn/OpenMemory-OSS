...
async function basicOperations() {
  ...
  const memory1 = await client.add(
    'I went to Paris yesterday and loved the Eiffel Tower',
  );
  console.log(
    `Memory stored in ${memory1.primary_sector} sector (${memory1.sectors.join(', ')}):`,
    memory1.id,
  );
  const memory2 = await client.add(
    'I feel really excited about the new AI project',
  );
  console.log(
    `Memory stored in ${memory2.primary_sector} sector (${memory2.sectors.join(', ')}):`,
    memory2.id,
  );
  const memory3 = await client.add(
    'My morning routine: coffee, then check emails, then code',
  );
  console.log(
    `Memory stored in ${memory3.primary_sector} sector (${memory3.sectors.join(', ')}):`,
    memory3.id,
  );
...
async function sectorOperations() {
  ...
  const emotionalMemories = await client.querySector(
    'happiness and joy',
    'emotional',
  );
  console.log('Emotional memories:', emotionalMemories.matches.length);
  const proceduralMemories = await client.querySector(
    'morning routine',
    'procedural',
  );
  console.log('Procedural memories:', proceduralMemories.matches.length);
  const episodicMemories = await client.querySector(
    'yesterday events',
    'episodic',
  );
...
async function sectorClassificationExamples() {
  ...
  for (const example of examples) {
    ...
    console.log(
      `Expected: ${example.expectedSector}, Actual: ${result.primary_sector}`,
    );
    console.log('---');
  }
}
