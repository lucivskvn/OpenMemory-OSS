...
process.env.OM_DB_PATH = path.join(
  tmpDir,
  `openmemory-observe-${process.pid}-${Date.now()}.sqlite`,
);
...
describe('DB Observability Wrappers', () => {
  ...
  afterEach(() => {
    ...
    if (_prev_log === undefined) delete process.env.OM_DB_LOG;
    else process.env.OM_DB_LOG = _prev_log;
    if (_prev_slow === undefined) delete process.env.OM_DB_SLOW_MS;
    else process.env.OM_DB_SLOW_MS = _prev_slow;
  });
