...
const nodeModulesNextPkg = path.join(
  root,
  'node_modules',
  'next',
  'package.json',
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      if (compareVersion(bunVersion, latestBunVersion) < 0) {
        fail(
          `Bun version is outdated. Latest is ${latestBunVersion}, installed is ${bunVersion}`,
        );
        errors++;
  ...
  try {
    ...
    if (nodeMajor && nodeMajor < MIN_NODE_MAJOR) {
      fail(
        `Node.js is older than ${MIN_NODE_MAJOR}. Some fallback features may not work. Node >= ${MIN_NODE_MAJOR} recommended`,
      );
      errors++;
    } else {
    ...
    if (fs.existsSync(nodeModulesAiPkg)) {
      ...
      try {
        ...
        if (latest !== aiPkg.version) {
          fail(
            `AI SDK version is outdated. Latest is ${latest}, installed is ${aiPkg.version}`,
          );
          errors++;
        } else {
  ...
  try {
    ...
    try {
      ...
      if (
        major < MIN_NEXT_MAJOR ||
        compareFn(nextVer.replace(/\^|#/, ''), latest) < 0
      ) {
        fail(
          `Next.js version outdated or unsupported. Latest is ${latest}, installed is ${nextVer}. Prefer Next.js ${MIN_NEXT_MAJOR}+`,
        );
        errors++;
      } else {
      ...
      if (!major || major < MIN_NEXT_MAJOR) {
        fail(
          `Next.js < ${MIN_NEXT_MAJOR} might not be supported; prefer Next.js ${MIN_NEXT_MAJOR}+`,
        );
        errors++;
      } else {
  ...
  async function tryImport(
    ...
    try {
      ...
      for (const sym of requiredSymbols) {
        if (
          typeof modObj[sym] === 'function' ||
          typeof modObj[sym] === 'undefined'
        ) {
          if (sym in modObj) {
      ...
      fail(
        `Failed to import ${mod}: ${err instanceof Error ? err.message : String(err)}`,
      );
      errors++;
    }
  }
  ...
  try {
    ...
    if (typeof ai.generateText === 'function') {
      try {
        ...
        fail(
          'generateText should require valid model, but succeeded with mock',
        );
        errors++;
      } catch (err) {
  ...
  try {
    ...
    if (
      ...
      ok('TextEncoder/TextDecoder available');
    else {
      fail('TextEncoder/TextDecoder not available');
      errors++;
    }
    ...
    if (typeof EventSource !== 'undefined') ok('EventSource available');
      ...
      console.warn(
        '⚠️ EventSource not available; using polyfill for SSE compatibility',
      );
  } catch (err) {
  ...
  try {
    ...
    try {
      const aiPkgPath = path.join(
        path.dirname(nodeModulesAiPkg || ''),
        'package.json',
      );
      if (fs.existsSync(aiPkgPath)) {
        ...
        if (
          aiPkg?.exports &&
          (aiPkg.exports['./rsc'] || aiPkg.exports['./rsc.js'])
        ) {
          hasRSCExport = true;
        }
      }
      ...
      try {
        ...
        if (typeof createStreamableValue === 'function') {
          ...
          try {
            ...
            fail(
              'createStreamableValue instantiation failed: ' + String(innerErr),
            );
            errors++;
          }
        } else {
          ...
          ok(
            'ai/rsc present but createStreamableValue not exported - skipping',
          );
        }
      } catch (rscErr) {
        // Do not treat this as an error; RSC streaming is optional for OpenMemory
        console.log(
          'ai/rsc subpath not resolvable; skipping RSC streaming check',
        );
      }
    } else {
      // Do not warn — this is optional and many environments don't support it.
      ok(
        'ai/rsc subpath not found in ai package exports; RSC streaming is optional',
      );
    }
    ...
    if (typeof streamText === 'function') {
      ...
      try {
        ...
        if (textStream) {
          ...
          console.warn(
            '⚠️ streamText did not return a textStream in mock test; continuing',
          );
        }
      } catch (innerErr) {
  ...
  try {
    ...
    try {
      ...
      if (
        routeContent.includes('streamText') &&
        routeContent.includes("from 'ai'")
      ) {
      ...
      if (routeContent.includes('.toUIMessageStreamResponse(')) {
        ok(
          'toUIMessageStreamResponse method used in /api/chat route (AI SDK primary with fallback)',
        );
      } else {
        fail(
          'toUIMessageStreamResponse method call not found in /api/chat route - AI SDK integration incomplete',
        );
        errors++;
      }
