...
export function activate(context: vscode.ExtensionContext) {
  ...
  const status_click = vscode.commands.registerCommand(
    'openmemory.statusBarClick',
    () => show_menu(),
  );
  const query_cmd = vscode.commands.registerCommand(
    'openmemory.queryContext',
    async () => {
  ...
  const patterns_cmd = vscode.commands.registerCommand(
    'openmemory.viewPatterns',
    async () => {
      if (!session_id) {
      ...
      try {
        ...
        const doc = await vscode.workspace.openTextDocument({
          content: format_patterns(patterns),
          language: 'markdown',
        });
        await vscode.window.showTextDocument(doc);
      } catch (error) {
  ...
  const toggle_cmd = vscode.commands.registerCommand(
    'openmemory.toggleTracking',
    () => {
      is_tracking = !is_tracking;
      update_status_bar(is_tracking ? 'active' : 'paused');
      vscode.window.showInformationMessage(
        `Tracking ${is_tracking ? 'enabled' : 'paused'}`,
      );
    },
  );

  const setup_cmd = vscode.commands.registerCommand('openmemory.setup', () =>
    show_quick_setup(),
  );
...
async function auto_link_all() {
  ...
  try {
    ...
    configs.push(
      await writeCursorConfig(backend_url, api_key, use_mcp, mcp_server_path),
    );
    configs.push(
      await writeClaudeConfig(backend_url, api_key, use_mcp, mcp_server_path),
    );
    configs.push(
      await writeWindsurfConfig(backend_url, api_key, use_mcp, mcp_server_path),
    );
    configs.push(
      await writeCopilotConfig(backend_url, api_key, use_mcp, mcp_server_path),
    );
    configs.push(
      await writeCodexConfig(backend_url, api_key, use_mcp, mcp_server_path),
    );

    const mode = use_mcp ? 'MCP protocol' : 'Direct HTTP';
    vscode.window.showInformationMessage(
      `✅ Auto-linked OpenMemory to AI tools (${mode})`,
    );
    auto_linked = true;
  } catch (error) {
...
function update_status_bar(
  state: 'active' | 'paused' | 'connecting' | 'disconnected' | 'disabled',
) {
  const icons = {
    active: '$(pulse) OpenMemory',
    paused: '$(debug-pause) OpenMemory',
    connecting: '$(sync~spin) OpenMemory',
    disconnected: '$(error) OpenMemory',
    disabled: '$(circle-slash) OpenMemory',
  };
...
async function show_menu() {
  if (!is_enabled) {
    ...
    if (choice.action === 'enable') {
      ...
      is_enabled = true;
      vscode.window.showInformationMessage(
        'OpenMemory enabled. Reloading window...',
      );
  ...
  items.push(
    is_tracking
      ? { label: '$(debug-pause) Pause Tracking', action: 'pause' }
      : { label: '$(play) Resume Tracking', action: 'resume' },
  );
  items.push(
    { label: '$(search) Query Context', action: 'query' },
    { label: '$(graph) View Patterns', action: 'patterns' },
    {
      label: use_mcp
        ? '$(link) Switch to Direct HTTP'
        : '$(server-process) Switch to MCP Mode',
      action: 'toggle_mcp',
    },
    { label: '$(circle-slash) Disable Extension', action: 'disable' },
    { label: '$(gear) Setup', action: 'setup' },
    { label: '$(refresh) Reconnect', action: 'reconnect' },
  );
  const choice = await vscode.window.showQuickPick(items, {
    placeHolder: 'OpenMemory Actions',
  });
  if (!choice) return;
  switch (choice.action) {
    case 'pause':
      is_tracking = false;
      update_status_bar('paused');
      break;
    case 'resume':
      is_tracking = true;
    ...
    case 'toggle_mcp':
      ...
      vscode.window.showInformationMessage(
        `Switched to ${use_mcp ? 'MCP' : 'Direct HTTP'} mode. Reconnecting...`,
      );
      await auto_link_all();
      break;
    case 'disable':
...
async function show_quick_setup() {
  const items = [
    {
      ...
      description: is_enabled
        ? 'Turn off OpenMemory tracking'
        : 'Turn on OpenMemory tracking',
    },
    {
      label: '$(server-process) Toggle MCP Mode',
      action: 'mcp',
      description: use_mcp
        ? 'Currently: MCP (switch to Direct HTTP)'
        : 'Currently: Direct HTTP (switch to MCP)',
    },
    { label: '$(key) Configure API Key', action: 'apikey' },
    { label: '$(server) Change Backend URL', action: 'url' },
    {
      label: '$(file-code) Set MCP Server Path',
      action: 'mcppath',
      description: 'Optional: custom MCP server executable',
    },
  ...
  switch (choice.action) {
    ...
    case 'mcp':
      ...
      vscode.window.showInformationMessage(
        `Switched to ${use_mcp ? 'MCP' : 'Direct HTTP'} mode`,
      );
      await auto_link_all();
      break;
    case 'mcppath':
      const path = await vscode.window.showInputBox({
        prompt:
          'Enter MCP server executable path (leave empty to use backend MCP)',
        value: mcp_server_path,
    ...
    case 'docs':
      vscode.env.openExternal(
        vscode.Uri.parse('https://github.com/lucivskvn/openmemory-OSS'),
      );
      break;
    case 'test':
      update_status_bar('connecting');
...
async function start_session() {
  try {
    ...
    const response = await fetch(`${backend_url}/api/ide/session/start`, {
      method: 'POST',
      headers: get_headers(),
      body: JSON.stringify({
        user_id: 'vscode-user',
...
async function end_session() {
  ...
  try {
    await fetch(`${backend_url}/api/ide/session/end`, {
      method: 'POST',
      headers: get_headers(),
...
async function send_event(event_data: {
  ...
  try {
    await fetch(`${backend_url}/api/ide/events`, {
      method: 'POST',
      headers: get_headers(),
      body: JSON.stringify({
        session_id,
...
async function query_context(query: string, file: string) {
  const response = await fetch(`${backend_url}/api/ide/context`, {
    method: 'POST',
    headers: get_headers(),
...
async function get_patterns(sid: string) {
  const response = await fetch(`${backend_url}/api/ide/patterns/${sid}`, {
    method: 'GET',
    headers: get_headers(),
  });
  const data = await response.json();
