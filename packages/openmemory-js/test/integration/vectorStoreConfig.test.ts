...
describe("Vector Store Config Consistency", () => {
    const originalVectorTable = Bun.env.OM_VECTOR_TABLE;

    beforeEach(() => {
        // Reset env vars
        delete Bun.env.OM_VECTOR_TABLE;
    });

    afterEach(() => {
        Bun.env.OM_VECTOR_TABLE = originalVectorTable;
        reloadConfig();
    });
    ...
    test("Custom Configuration overrides correctly", () => {
        Bun.env.OM_VECTOR_TABLE = "custom_vectors";
        reloadConfig();
