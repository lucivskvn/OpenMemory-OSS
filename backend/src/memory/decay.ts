...
const parse_int = (x: any, d: number) => Number.isFinite(+x) ? Math.floor(+x) : d
const parse_f = (x: any, d: number) => Number.isFinite(+x) ? +x : d
const parse_bool = (x: any, d: boolean) => x === 'true' ? true : x === 'false' ? false : d
const clamp_f = (v: number, a: number, b: number) => Math.min(b, Math.max(a, v))
const clamp_i = (v: number, a: number, b: number) => Math.min(b, Math.max(a, Math.floor(v)))
const tick = () => new Promise<void>(r => setImmediate(r))
const sleep = (ms: number) => new Promise(r => setTimeout(r, ms))

const mean = (arr: number[]) => arr.length ? arr.reduce((a, b) => a + b, 0) / arr.length : 0
const l2 = (v: number[]) => Math.sqrt(v.reduce((s, x) => s + x * x, 0))
const normalize = (v: number[]) => { const n = l2(v) || 1; for (let i = 0; i < v.length; i++) v[i] /= n; return v }
const chunkz = <t>(arr: t[], k: number) => {
    const n = Math.max(1, k | 0), out: t[][] = Array.from({ length: n }, () => [])
...
const compress_vector = (vec: number[], f: number, min_dim = 64, max_dim = 1536): number[] => {
    ...
    for (let i = 0; i < src.length; i += bucket) pooled.push(mean(src.slice(i, i + bucket)))
    normalize(pooled)
    return pooled
}
...
const stop = new Set([
    'the', 'a', 'an', 'to', 'of', 'and', 'or', 'in', 'on', 'for', 'with', 'at', 'by', 'is', 'it', 'be', 'as', 'are', 'was', 'were',
    'from', 'that', 'this', 'these', 'those', 'but', 'if', 'then', 'so', 'than', 'into', 'over', 'under', 'about', 'via', 'vs', 'not'
])

const top_keywords = (t: string, k = 5): string[] => {
    const words = (t.toLowerCase().match(/[a-z0-9]+/g) || []).filter(w => !stop.has(w))
...
export const apply_decay = async () => {
    ...
    let tot_proc = 0, tot_chg = 0, tot_comp = 0, tot_fp = 0
    ...
    for (const seg of segments) {
        ...
        await Promise.all(parts.map(async (part) => {
            for (const m of part) {
                ...
                const lam = tier === 'hot' ? cfg.lambda_hot : tier === 'warm' ? cfg.lambda_warm : cfg.lambda_cold
                const dt = Math.max(0, (now_ts - (m.last_seen_at || m.updated_at)) / cfg.time_unit_ms)
                const act = Math.max(0, m.coactivations || 0)
                const sal = clamp_f((m.salience || 0.5) * (1 + Math.log1p(act)), 0, 1)
                ...
                if (f < 0.7) {
                    ...
                    if (vec_row && vec_row.vector) {
                        ...
                        const before_len = Array.isArray(vec) ? vec.length : 0

                        if (before_len > 0) {
                            const new_vec = compress_vector(vec, f, cfg.min_vec_dim, cfg.max_vec_dim)
