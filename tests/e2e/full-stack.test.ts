...
describe('Full stack E2E tests (backend + dashboard integration)', () => {
  ...
  it('dashboard API telemetry & settings integration', async () => {
    ...
    const embedConfig = await fetch(`${baseUrl}/embed/config`).then((r) =>
      r.json(),
    );
  ...
  it('AI SDK streaming / chat (dashboard route import)', async () => {
    ...
    const req: any = {
      ...
      json: async () => ({
        messages: [{ role: 'user', content: 'Hello OpenMemory' }],
      }),
  ...
  it('multi-tenant isolation: different users see only their memories', async () => {
    ...
    expect(results1.matches.some((m: any) => m.id === idByUser['u1'])).toBe(
      true,
    );
    expect(results1.matches.some((m: any) => m.id === idByUser['u2'])).toBe(
      false,
    );
    ...
    expect(results2.matches.some((m: any) => m.id === idByUser['u2'])).toBe(
      true,
    );
    expect(results2.matches.some((m: any) => m.id === idByUser['u1'])).toBe(
      false,
    );
  });
  ...
  it('temporal graph integration: facts timeline & queries', async () => {
    ...
    const timelineRes = await fetch(
      `${baseUrl}/api/temporal/timeline?subject=test_subject`,
    );
