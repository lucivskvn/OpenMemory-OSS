...
## 2026-07-23 - [Unauthenticated/Unauthorized Global Classifier Training Bypass]
**Vulnerability:** The `/api/system/classifier/train` endpoint allowed any unauthenticated client (or regular tenants) to trigger the CPU and memory-intensive classifier training process with arbitrary payloads, posing a Denial of Service (DoS) and model manipulation/corruption risk.
**Learning:** System-wide maintenance or configuration endpoints that don't directly handle user-scoped memory objects are easily overlooked for authentication and administrative-scoped authorization checks, exposing system resources to external abuse.
**Prevention:** Always enforce `require_tenant` authentication and restrict system-wide/model-training endpoints to administrator or global system tenants (`admin`, `system`, or `dev-no-auth`).
