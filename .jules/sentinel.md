...
## 2026-08-04 - [Multi-Tenant Isolation Bypass in Python MCP Tool Handlers]
**Vulnerability:** In `packages/openmemory-py/src/openmemory/ai/mcp.py`, tool call handlers `openmemory_query`, `openmemory_store`, and `openmemory_list` extracted `uid = args.get("user_id")` directly from arguments without validating against the session tenant using `_resolve_mcp_tenant(mem, args)`. This allowed callers to query, store, or list memories across arbitrary tenants or bypass identity checks in unauthenticated sessions.
**Learning:** Tool execution handlers in AI agent protocol adapters (like MCP) must systematically resolve and validate the session tenant before processing tool arguments. Relying on caller-supplied `user_id` parameters without cross-checking the session identity creates multi-tenant isolation bypasses.
**Prevention:** Always pass `args` to `_resolve_mcp_tenant(mem, args)` across all MCP tool handlers, fail closed on unauthenticated sessions or tenant mismatches, and bind execution strictly to the verified session tenant.
