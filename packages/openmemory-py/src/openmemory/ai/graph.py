...
async def store_node_mem(p: LgmStoreReq) -> LgStoreResult:
    ...
    tag_list = build_tags(p.tags, node, ns, p.graph_id)
    ...
    stored = GraphMemoryItem(
        ...
        last_seen_at=res["created_at"],
        salience=res.get("salience", 0.5),
        user_id=user_id,
        simhash=res.get("simhash"),
        generated_summary=res.get("generated_summary")
    )
...
async def retrieve_node_mems(p: LgmRetrieveReq) -> LgRetrieveResult:
    ...
    if p.query:
        ...
        for m in matches:
             ...
             hyd = GraphMemoryItem(
                 ...
                 metadata=m_meta if inc_meta else {}, # alias meta? GraphMemoryItem inherits MemoryItem which maps metadata->meta
                 created_at=m["created_at"],
                 updated_at=m.get("updated_at", 0),
                 last_seen_at=m.get("last_seen_at", 0),
        ...
        for row in rows:
            ...
            row_meta = parse_json_safe(row.metadata, {})
            if not matches_ns(row_meta, ns, gid): continue
            ...
            hyd = GraphMemoryItem(
                 ...
                 metadata=row_meta if inc_meta else {},
