...
export function EmbeddingModeSelector({
  config,
  onConfigChange,
  className,
}: EmbeddingModeSelectorProps) {
  const [currentConfig, setCurrentConfig] = useState<EmbeddingConfig | null>(
    config || null,
  );
  ...
  const handleProviderChange = async (
    ...
    try {
      ...
      if (provider === 'router_cpu') {
        ...
        if (
          routerFallback !== undefined &&
          typeof routerFallback !== 'boolean'
        ) {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Unknown error occurred while updating provider';
  ...
  const handleGlobalSimdChange = async (globalSimd: boolean) => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Unknown error occurred while updating global SIMD';
      setError(errorMessage);
  ...
  const handleRouterSimdChange = async (routerSimd: boolean) => {
    ...
    try {
      ...
      if (routerSimd && !currentConfig.simd_global_enabled) {
        toast.warning(
          'Router SIMD enabled but global disabled - fusion ops may be slower',
        );
      }
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Unknown error occurred while updating router SIMD';
      setError(errorMessage);
  ...
  const handleRouterFallbackChange = async (routerFallback: boolean) => {
    ...
    try {
      ...
      toast.success(
        `Router fallback ${routerFallback ? 'enabled' : 'disabled'}`,
      );
    } catch (err) {
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Unknown error occurred while updating router fallback';
      setError(errorMessage);
  ...
  const handleEmbedModeChange = async (embedMode: 'simple' | 'advanced') => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Unknown error occurred while updating batch mode';
      setError(errorMessage);
        ...
        {currentConfig && (
          <div className="flex flex-col gap-2 mb-4">
            <div className="flex items-center gap-2">
              <Badge
                variant={updating ? 'secondary' : 'default'}
              ...
              <Badge variant="outline" className="flex items-center gap-1">
                ...
                {'Batching: ' +
                  (BATCH_MODE_OPTIONS.find(
                    (m) => m.value === currentConfig.batchMode,
                  )?.label || currentConfig.batchMode)}
              </Badge>
            </div>
            <div className="flex flex-wrap gap-1 items-center">
              ...
              <Badge
                variant={
                  currentConfig.simd_global_enabled ? 'default' : 'secondary'
                }
                className="text-xs"
              >
                Global SIMD:{' '}
                {currentConfig.simd_global_enabled ? 'Enabled' : 'Disabled'}
              </Badge>
              {currentConfig.provider === 'router_cpu' && (
                <Badge
                  variant={
                    currentConfig.simd_router_enabled ? 'default' : 'secondary'
                  }
                  className="text-xs"
                >
                  Router SIMD:{' '}
                  {currentConfig.simd_router_enabled
                    ? 'Enabled (+20-30% in routing)'
                    : 'Disabled'}
                </Badge>
              )}
            </div>
        ...
        {currentConfig && (
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                handleEmbedModeChange(value as 'simple' | 'advanced')
              }
          ...
          <div className="flex items-center justify-between p-3 border rounded-lg">
            <div className="space-y-0.5">
              <label className="text-sm font-medium">
                Global SIMD Optimization
              </label>
              <p className="text-xs text-muted-foreground">
                Enable SIMD for fusion and general vector ops across all
                providers (10-15% overall embedding speedup)
              </p>
            </div>
            <Switch
        ...
        {currentConfig?.provider === 'router_cpu' && (
          <div className="space-y-3 p-4 border rounded-lg bg-muted/20">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <label className="text-sm font-medium">
                  Router SIMD Optimization
                </label>
                <p className="text-xs text-muted-foreground">
                  Enable SIMD for sector routing operations (+20-30% in
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  handleRouterFallbackChange(checked)
                }
                disabled={updating || loading || restarting}
              />
            </div>
              ...
              <div className="grid grid-cols-3 gap-4 pt-2 border-t">
                <div className="text-center">
                  ...
                  <div className="text-sm font-medium">
                    {currentConfig.performance.expected_p95_ms}ms
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-xs text-muted-foreground">
                  ...
                  <div className="text-sm font-medium">
                    +{currentConfig.performance.expected_simd_improvement}%
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-xs text-muted-foreground">RAM Usage</div>
                  <div className="text-sm font-medium">
                    {currentConfig.performance.memory_usage_gb}GB
                  </div>
                </div>
              </div>
            )}
            ...
            {currentConfig.ollama_required && (
              <Alert>
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  Router mode requires Ollama service. Ensure Ollama is running
