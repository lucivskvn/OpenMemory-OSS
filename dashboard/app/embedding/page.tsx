...
import { getEmbeddingConfig, updateEmbeddingProvider, updateEmbeddingBatchMode, PROVIDER_OPTIONS, BATCH_MODE_OPTIONS } from "@/lib/api"
...
export default function EmbeddingDashboardPage() {
    ...
    const fetchOllamaStatus = useCallback(async () => {
        try {
            ...
            setOllamaStatus({ available: false, ollama_available: false, models_loaded: 0, status: 'error' })
        }
    }, [])
        ...
        <div className="min-h-screen max-w-6xl mx-auto p-6">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center space-x-3">
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="text-white font-medium">{providerInfo.label}</span>
                            </div>
                        </div>
                        <div className="flex items-center space-x-3">
                            <div className="text-stone-400">Batch Mode:</div>
                            <span className="text-white capitalize">{config?.batchMode || 'simple'}</span>
                        </div>
                        <div className="flex items-center space-x-3">
                        ...
                        {config?.model && (
                            <div className="flex items-center space-x-3">
                                ...
                                <span className="text-white font-mono text-sm">{config.model}</span>
                            </div>
                        )}
                        {config?.performance && (
                            <div className="flex items-center space-x-3">
                                <div className="text-stone-400">Expected P95:</div>
                                <span className="text-white">{config.performance.expected_p95_ms}ms</span>
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        {ollamaStatus && (
                            <>
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        <div className={`w-2 h-2 rounded-full ${ollamaStatus.available ? 'bg-green-500' : 'bg-red-500'}`}></div>
                                ...
                                <div className="flex items-center space-x-3">
                                    ...
                                    <span className="text-white">{ollamaStatus.models_loaded || 0}</span>
                                </div>
                                {ollamaStatus.ollama_version && (
                                    <div className="flex items-center space-x-3">
                                        <div className="text-stone-400">Version:</div>
                                        <span className="text-white font-mono text-sm">{ollamaStatus.ollama_version}</span>
                                    </div>
                                )}
                                {ollamaStatus.error && (
            ...
            <div className="bg-stone-950 rounded-xl border border-stone-800 p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label className="text-stone-400 text-sm mb-2 block">Embedding Provider</label>
                        <select
                        ...
                        {updating && <div className="text-yellow-400 text-sm mt-2">Updating configuration...</div>}
                    </div>

                    <div>
                        <label className="text-stone-400 text-sm mb-2 block">Batch Processing Mode</label>
                        <select
                        ...
                        <div className="text-stone-400 text-xs mt-2">
                            {BATCH_MODE_OPTIONS.find(m => m.value === (config?.batchMode || 'simple'))?.description}
                        </div>
                    </div>
                </div>
            </div>
            ...
            {ollamaStatus?.available && (
                <div className="bg-stone-950 rounded-xl border border-stone-800 p-6 mb-6">
                    ...
                    <div className="space-y-2 max-h-80 overflow-y-auto">
                        {ollamaModels.length === 0 ? (
                            ...
                            ollamaModels.map((model) => (
                                <div key={model.name} className="flex items-center justify-between p-3 bg-stone-900/50 rounded-lg">
                                    <div className="flex flex-col">
                                        <div className="text-stone-200 font-medium">{model.name}</div>
                                        {model.size && (
                    ...
                    <div className="mt-4 pt-4 border-t border-stone-800">
                        <div className="text-stone-400 text-sm">
                            Models shown here are available in your Ollama instance. Use the provider selector above to switch which model is used for embeddings.
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="bg-stone-950 rounded-xl border border-stone-800 p-6">
                ...
                <div className="space-y-4">
                    <div>
                        <label className="text-stone-400 text-sm block mb-2">Test Query</label>
                        <textarea
                            value={testInput}
                    ...
                    {testResult && (
                        <div className="bg-stone-900 rounded-xl p-4 border border-stone-700">
                            ...
                            <div className="text-stone-200 font-mono text-sm">{testResult}</div>
                        </div>
                    )}
                </div>
            </div>
