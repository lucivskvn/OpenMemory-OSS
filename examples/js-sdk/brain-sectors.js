...
async function sectorExample() {
  ...
  try {
    ...
    for (const test of testMemories) {
      ...
      console.log(
        `   Expected: ${test.expectedSector}, Got: ${memory.primary_sector}`,
      );
    ...
    const episodicResults = await client.querySector(
      'morning coffee',
      'episodic',
    );
    ...
    const emotionalResults = await client.querySector(
      'excited happy',
      'emotional',
    );
    ...
    const crossResults = await client.query('learning and feeling excited', {
      k: 10,
    });
    ...
    crossResults.matches.forEach((match, i) => {
      ...
      console.log(
        `      Score: ${match.score.toFixed(3)}, Path: [${match.path.join(' → ')}]`,
      );
    });
