...
describe('embedding utilities performance and correctness', () => {
  ...
  it('SIMD vector operations maintain correctness', () => {
    ...
    const expectedNormalizedFirst =
      rawWeighted / Math.sqrt(dim * rawWeighted * rawWeighted);
  ...
  if (process.env.OM_RUN_PERF_TESTS === 'true') {
    it('gen_syn_emb is reasonably fast for small inputs', () => {
      const t =
        'This is a short test string to measure synthetic embedding performance.';
      const runs = 200;
  ...
  it('embedding config endpoint has acceptable latency', async () => {
    ...
    console.log(
      `Config endpoint latency - avg: ${avgLatency.toFixed(2)}ms, p95: ${p95Latency.toFixed(2)}ms`,
    );
  });
  ...
  it('embedding processing has bounded latency', async () => {
    ...
    console.log(
      `Text processing latency - avg: ${avgProcessingTime.toFixed(2)}ms, max: ${maxProcessingTime.toFixed(2)}ms`,
    );
  });
  ...
  describe('Real P95 Latency Assertions', () => {
    it('synthetic embedding baseline <20ms with real P95 <200ms CI bounds', async () => {
      ...
      console.log(
        `Synthetic P95 latency - p95: ${p95.toFixed(2)}ms, avg: ${avg.toFixed(2)}ms`,
      );
    });
    ...
    it('SIMD ratio maintains 1.1-3.0x performance improvement with flakiness mitigation', async () => {
      ...
      for (const config of testConfigs) {
        ...
        const benchmark = await benchmarkSimd(
          config.dimension,
          config.iterations,
        );
      ...
      const p95Ratio = [...ratios].sort((a, b) => a - b)[
        Math.floor(ratios.length * 0.95)
      ];
      const ratioVariance =
      ...
      console.log(
        `SIMD performance ratio - avg: ${avgRatio.toFixed(2)}x, p95: ${p95Ratio.toFixed(2)}x, std: ${Math.sqrt(ratioVariance).toFixed(3)}`,
      );
    });
    ...
    it('concurrent load testing (50 requests) achieves <300ms total throughput', async () => {
      ...
      const testText =
        'Concurrent load test text for throughput verification. This should maintain reasonable performance under parallel processing load.';
      ...
      const p95RequestTime = [...results].sort((a, b) => a - b)[
      ...
      console.log(
        `Concurrent load - total: ${totalElapsed.toFixed(2)}ms, avg_req: ${avgRequestTime.toFixed(2)}ms, p95_req: ${p95RequestTime.toFixed(2)}ms`,
      );
    });
    ...
    it('baseline stability testing ensures router/cpu <1.5x synthetic latency', async () => {
      ...
      const syntheticAvg =
        syntheticLatencies.reduce((sum, lat) => sum + lat, 0) /
        syntheticLatencies.length;
      const syntheticP95 = [...syntheticLatencies].sort((a, b) => a - b)[
        Math.floor(syntheticLatencies.length * 0.95)
      ];

      console.log(
        `Synthetic baseline - avg: ${syntheticAvg.toFixed(2)}ms, p95: ${syntheticP95.toFixed(2)}ms`,
      );
      ...
      if (embeddingInfo.kind === 'router_cpu' && embeddingInfo.simd_enabled) {
        ...
        for (let i = 0; i < Math.min(iterations, 25); i++) {
          // Reduce iterations for complex providers
          const start = performance.now();
        ...
        const routerAvg =
          routerLatencies.reduce((sum, lat) => sum + lat, 0) /
          routerLatencies.length;
        const routerP95 = [...routerLatencies].sort((a, b) => a - b)[
          Math.floor(routerLatencies.length * 0.95)
        ];

        console.log(
          `Router CPU performance - avg: ${routerAvg.toFixed(2)}ms, p95: ${routerP95.toFixed(2)}ms`,
        );
        ...
        console.log(
          `Relative performance - avg_ratio: ${(routerAvg / syntheticAvg).toFixed(2)}x, p95_ratio: ${(routerP95 / syntheticP95).toFixed(2)}x`,
        );
      } else {
        console.log(
          `Router CPU stability test skipped (not using router_cpu with SIMD)`,
        );
      }
    });
    ...
    it('endpoint latency with real performance.now() P95 calculations', async () => {
      ...
      const avgLatency =
        endpointLatencies.reduce((sum, lat) => sum + lat, 0) /
        endpointLatencies.length;
      const p95Latency =
        endpointLatencies[Math.floor(endpointLatencies.length * 0.95)];
      const p99Latency =
      ...
      console.log(
        `Real endpoint P95 latency - avg: ${avgLatency.toFixed(2)}ms, p95: ${p95Latency.toFixed(2)}ms, p99: ${p99Latency.toFixed(2)}ms`,
      );
    });
  });
});
