...
async function get_db_version_sqlite(db: sqlite3.Database): Promise<string | null> {
...
async function set_db_version_sqlite(db: sqlite3.Database, version: string): Promise<void> {
    return new Promise((ok, no) => {
        db.run(
...
async function run_sqlite_migration(db: sqlite3.Database, m: Migration): Promise<void> {
...
async function check_column_exists_pg(pool: Pool, table: string, column: string): Promise<boolean> {
    const sc = process.env.OM_PG_SCHEMA || 'public'
