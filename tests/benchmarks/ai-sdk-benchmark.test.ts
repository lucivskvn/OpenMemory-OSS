...
describe('AI SDK streaming benchmark', () => {
  it('measures TTFT, TPS, and total time for dashboard chat endpoint', async () => {
    ...
    try {
      ...
      if (!allowBuild) {
        console.warn(
          'Dashboard not running on localhost:3000 and OM_ALLOW_DASHBOARD_BUILD not set. Skipping AI SDK benchmark (gated by OM_RUN_PERF_TESTS).',
        );
        console.warn(
          'For local runs, either: (1) start dashboard manually or (2) set OM_ALLOW_DASHBOARD_BUILD=true to let test manage lifecycle',
        );
        return;
      }
      // Dashboard not running but build allowed - start it (resource intensive!)
      console.log(
        'Building dashboard (OM_ALLOW_DASHBOARD_BUILD=true allows this)...',
      );
      ...
      await new Promise<void>((resolve, reject) => {
        buildProc.on('close', (code: number) =>
