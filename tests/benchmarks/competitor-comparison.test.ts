...
if (process.env.OM_RUN_BENCHMARK_TESTS !== 'true') {
    console.log('Skipping competitor benchmarks (set OM_RUN_BENCHMARK_TESTS=true to run)');
}
...
describe('competitor benchmarks (OpenMemory vs Mem0/Zep)', () => {
    ...
    it('should create synthetic dataset and run recall/latency/throughput benchmarks', async () => {
        ...
        const numMemories = parseInt(process.env.OM_BENCHMARK_MEMORIES || '1000', 10);
        ...
        for (let i = 0; i < numMemories; i++) {
            ...
            const res = await fetch(`${baseUrl}/memory/add`, {
                ...
                body: JSON.stringify({ content, metadata: { primary_sector: 'semantic' } }),
            });
        ...
        for (const qq of queries) {
            ...
            if (matches.length > 0 && matches[0].id === qq.expectedId) recallCounts.k1++;
        ...
        const concurrent = parseInt(process.env.OM_BENCHMARK_CONCURRENT || '50', 10);
        const qpsStart = performance.now();
        await Promise.all(Array.from({ length: concurrent }).map(async (_, i) => {
        ...
        const file = path.join(resultsDir, `competitor-comparison-${Date.now()}.json`);
