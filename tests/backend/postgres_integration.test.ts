...
test(
  'postgres integration: start server and crud memory',
  async () => {
    if (process.env.OM_METADATA_BACKEND !== 'postgres') {
      if (process.env.TEST_DEBUG === '1')
        console.log(
          'Skipping Postgres integration test (OM_METADATA_BACKEND != postgres)',
        );
      return;
    }
    ...
    if (process.env.TEST_DEBUG === '1')
      console.log(
        `Running Postgres integration test against ${user}@${host}:${port}/${db}`,
      );
