...
test("postgres: OM_PG_CONNECTION_STRING parsing variants", async () => {
    if (process.env.OM_METADATA_BACKEND !== "postgres") {
        if (process.env.TEST_DEBUG === '1') console.log("Skipping Postgres tests (OM_METADATA_BACKEND != postgres)");
        return;
    }
            ...
            try {
                ...
                try {
                    ...
                    if (capturedOpts) {
                        ...
                        expect(capturedOpts.password).toBe(testCase.expectedOpts.password);
                        expect(capturedOpts.database).toBe(testCase.expectedOpts.database);

                        // Check SSL configuration based on sslmode
                        if (testCase.expectedSsl === false) {
                            expect(capturedOpts.ssl).toBe(false);
                        } else if (testCase.expectedSsl && typeof testCase.expectedSsl === 'object') {
                            expect(capturedOpts.ssl).toBeDefined();
                            expect((capturedOpts.ssl as any).rejectUnauthorized).toBe(testCase.expectedSsl.rejectUnauthorized);
                        } else {
                            expect(capturedOpts.ssl).toBeUndefined();
                        }
                    } else {
                        throw new Error("Expected postgres client to be initialized with connection string");
                    }
...
test("postgres: invalid OM_PG_CONNECTION_STRING falls back to env vars", async () => {
    if (process.env.OM_METADATA_BACKEND !== "postgres") {
        if (process.env.TEST_DEBUG === '1') console.log("Skipping Postgres tests (OM_METADATA_BACKEND != postgres)");
        return;
    }
