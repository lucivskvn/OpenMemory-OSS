...
export const run_reflection = async () => {
    console.log('[SERVER] REFLECT: Starting reflection job...')
    const min = env.reflect_min || 20
    const mems = await q.all_mem.all(100, 0)
    console.log(`[SERVER] REFLECT: Fetched ${mems.length} memories (min required: ${min})`)
    if (mems.length < min) {
        console.log('[SERVER] REFLECT: Not enough memories, skipping')
        return { created: 0, reason: 'low' }
    }
    const cls = cluster(mems)
    console.log(`[SERVER] REFLECT: Clustered into ${cls.length} groups`)
    let n = 0
    for (const c of cls) {
        const txt = summ(c)
        const s = sal(c)
        ...
        console.log(`[SERVER] REFLECT: Creating reflection: ${c.n} memories, salience=${s.toFixed(3)}, sector=${c.mem[0].primary_sector}`)
    ...
    console.log(`[SERVER] REFLECT: Job complete: created ${n} reflections`)
...
export const start_reflection = () => {
    ...
    timer = setInterval(() => run_reflection().catch(e => console.error('[SERVER] REFLECT:', e)), int)
    console.log(`[SERVER] REFLECT: Started: every ${env.reflect_interval || 10}m`)
}
