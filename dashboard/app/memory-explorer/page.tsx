          ...
          <table className="w-full">
            ...
            <tbody>
              ...
              {memories.map((memory) => (
                <tr
                  key={memory.id}
                  className="border-t border-stone-800 hover:bg-stone-900/50"
                >
                  <td className="px-4 py-3">
                    <input
                      ...
                      onChange={(e) =>
                        handleSelectMemory(memory.id, e.target.checked)
                      }
                      className="rounded border-stone-600"
                    />
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`bg-${sectorColors[memory.primary_sector] || 'gray'}-500/10 border border-${sectorColors[memory.primary_sector] || 'gray'}-500/20 text-${sectorColors[memory.primary_sector] || 'gray'}-400 px-2 py-1 rounded text-xs`}
                    >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-6">
          <div className="text-stone-400">
            ...
            {Math.min(currentPage * limit, totalCount)} of {totalCount} memories
          </div>
          <div className="flex gap-2">
            <button
      ...
      <div className="bg-stone-950 rounded-xl p-6 max-w-2xl w-full mx-4 border border-stone-800 max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-stone-400 text-sm block">
                Primary Sector
              </label>
              <span
                className={`bg-${sectorColors[memory.primary_sector] || 'gray'}-500/10 border border-${sectorColors[memory.primary_sector] || 'gray'}-500/20 text-${sectorColors[memory.primary_sector] || 'gray'}-400 px-2 py-1 rounded text-sm`}
              >
                {memory.primary_sector}
              </span>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {memory.tags.map((tag, i) => (
                  <span
                    key={i}
                    className="bg-stone-800 text-stone-300 px-2 py-1 rounded text-sm"
                  >
                    {tag}
                  </span>
                ))}
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
      <div className="bg-stone-950 rounded-xl p-6 max-w-md w-full mx-4 border border-stone-800">
        ...
        <p className="text-stone-400 mb-6">
          Are you sure you want to delete {count} selected memories? This action
          cannot be undone.
        </p>
