...
class ServerManager {
  ...
  async startDashboard(): Promise<void> {
    ...
    const startProcess = spawn(
      'bun',
      ['run', 'start', '-p', TEST_CONFIG.dashboardPort.toString()],
      {
        cwd: process.cwd() + '/dashboard',
        stdio: ['ignore', 'pipe', 'pipe'],
        detached: true,
      ...
      if (server.process && server.process.pid) {
        try {
          ...
          try {
            ...
            console.warn(
              `Fallback kill also failed for ${name}:`,
              fallbackError,
            );
          }
        }
  ...
  describe('Server Startup', () => {
    ...
    test('dashboard server starts successfully', async () => {
      ...
      try {
        ...
        // In mock environment, expect this to fail gracefully
        console.log(
          'Dashboard connectivity test (expected in mock env):',
          error,
        );
      }
    });
    ...
    test('message rendering format', () => {
      const messages = [
        { role: 'user', content: 'User message', id: '1' },
        {
          role: 'assistant',
          content:
            'Assistant response[[OM_TELEMETRY]]{"memory_ids":[]}[[/OM_TELEMETRY]]',
          id: '2',
        },
      ];
  ...
  describe('Error Scenarios', () => {
    ...
    test('memory query failure fallback', () => {
      ...
      const fallbackResponse =
        "I apologize, but I couldn't retrieve your memories at this time.";
