...
test("initDb() is idempotent across DB paths", async () => {
    ...
    const dbPath1 = path.join(tmpDir, `openmemory-initdb-${process.pid}-1.sqlite`);
    const dbPath2 = path.join(tmpDir, `openmemory-initdb-${process.pid}-2.sqlite`);

    // Ensure any previous artifacts removed
    try { fs.unlinkSync(dbPath1); } catch (e) { }
