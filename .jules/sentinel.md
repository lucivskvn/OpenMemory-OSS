...
## 2026-08-04 - [Missing Multi-Tenant User Summary Endpoints and Exception Leakage in Python Server]
**Vulnerability:** User summary routes (`GET /users/{user_id}/summary` and `POST /users/{user_id}/summary/regenerate`) existed in TypeScript with strict multi-tenant scoping, but were completely missing from the Python FastAPI server. Additionally, `Memory.history()` returned raw SQLite `bytes` fields (such as compressed vector blobs) without UTF-8/hex transformation, causing FastAPI response serialization to throw `UnicodeDecodeError` exceptions and fail endpoints.
**Learning:** In polyglot monorepos with feature parity requirements, missing secondary API endpoints create security gaps and inconsistent feature boundaries. Furthermore, returning raw database `bytes` types to FastAPI endpoints without safe UTF-8 decoding or hex fallback triggers framework-level serialization crashes.
**Prevention:** Maintain strict 1:1 route and security policy parity across TypeScript and Python runtimes, enforce tenant verification (`user_id == request.state.tenant`) and input length bounds on all user endpoints, and safely transform raw database `bytes` to string or hex before returning them in API responses.
