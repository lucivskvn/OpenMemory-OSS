...
if (typeof document !== 'undefined' && typeof window !== 'undefined') {
  describe('ChatInner Component - React RTL Tests', () => {
    test('renders ChatInner component with correct UI elements', async () => {
      ...
      expect(
        screen.getByPlaceholderText('Ask about your memories...'),
      ).toBeInTheDocument();
    ...
    test('displays streaming response with memory content', async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/Based on your stored knowledge/i),
          ).toBeInTheDocument();
        },
        { timeout: 15000 },
      );
      ...
      expect(
        screen.getByText(/AI technology involves machine learning algorithms/i),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/and neural network architectures/i),
      ).toBeInTheDocument();

      // Check completion metadata
      expect(
        screen.getByText(
          /- Retrieved 2 memories from 1 sectors • Confidence: high/i,
        ),
      ).toBeInTheDocument();
    });
  ...
  console.warn(
    'Skipping ChatInner DOM tests: document/window not available in this test environment',
  );
}
...
if (typeof document !== 'undefined' && typeof window !== 'undefined') {
  ...
  console.warn(
    'Skipping Memory Sidebar DOM tests: document/window not available',
  );
}
