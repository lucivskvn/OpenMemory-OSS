...
## 2026-09-09 - [Cross-Tenant Temporal Fact Mutation and Destruction]
**Vulnerability:** The temporal fact mutation helper functions (`update_fact`, `invalidate_fact`, and `delete_fact`) in both TypeScript and Python SDKs executed SQL `UPDATE` and `DELETE` queries solely filtered by `id=?` without enforcing tenant/`user_id` constraints. This allowed cross-tenant modification, invalidation, or destruction of temporal facts if a fact ID was known or guessed.
**Learning:** While point-lookup GET handlers and insertion logic enforced tenant boundaries, mutation and deletion helpers in graph store modules omitted `user_id` parameters, creating a multi-tenant authorization bypass at the database layer.
**Prevention:** Ensure all `UPDATE` and `DELETE` database query helpers explicitly accept a `user_id` parameter and append `AND user_id = ?` to prevent cross-tenant resource mutation.
