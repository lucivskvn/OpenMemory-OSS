...
describe("Database Migrations", () => {
    ...
    test("Run Migrations on :memory: DB", async () => {
        ...
        // Fix: Use a temp file for persistence, ensure absolute path to avoid mkdir issues
        const path = await import("path");
        const tempDb = path.join(process.cwd(), `test_migration_${Date.now()}.sqlite`);
        ...
        try {
            ...
            if (fs.existsSync(tempDb)) {
                ...
                try {
                    ...
                    console.error("Failed to cleanup temp migration db:", e);
                }
            }
