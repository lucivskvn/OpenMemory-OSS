    ...
    it('sends embedding_mode to backend /memory/query and returns SSE', async () => {
        ...
        globalThis.fetch = async (url: any, opts: any) => {
            if (typeof url === 'string' && url.endsWith('/memory/query')) {
                ...
                return {
                    ok: true,
                    json: async () => ({ matches: [{ id: 'm1', content: 'hello', score: 0.9 }] }),
                };
            }
            // Fallback
        ...
        const memRoute: any = await import('../../dashboard/app/api/chat/mem/route');
        // Create a mock NextRequest shape with nextUrl
        const req: any = { nextUrl: new URL('http://localhost/?q=test&embedding_mode=router_cpu') };
        const res: any = await memRoute.GET(req);
        expect(res).toBeTruthy();
        expect(capturedBody).not.toBeNull();
        expect(capturedBody.metadata && capturedBody.metadata.embedding_mode).toBe('router_cpu');
