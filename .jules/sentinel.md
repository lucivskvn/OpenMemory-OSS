...
## 2026-08-03 - [Raw Exception Message Leakage across Dashboard, Memory Query, and LangGraph Routes]
**Vulnerability:** Caught exceptions in `/dashboard/*` (`/projects`, `/stats`, `/health`, `/activity`, `/sectors/timeline`, `/top-memories`, `/maintenance`), `/memory/query`, and `/lgm/*` routes returned raw `e.message` details directly to callers in JSON responses (`message: e.message`), leaking internal database schema, SQL errors, and system trace details.
**Learning:** Exception handlers in API routes can easily leak internal system and database implementation details if `e.message` or `(e as Error).message` is included in error response payloads.
**Prevention:** Always log exceptions server-side with `console.error` or a logger, and return sanitized, generic HTTP 500 or 400 error responses (e.g. `{ err: "internal" }`) to HTTP clients.
