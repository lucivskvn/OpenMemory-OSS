...
export async function GET(req: NextRequest) {
    ...
    const url = `${API_BASE_URL}/dashboard?limit=${limit}&offset=${offset}` + (user_id ? `&user_id=${encodeURIComponent(user_id)}` : '') + (embedding_mode ? `&embedding_mode=${encodeURIComponent(embedding_mode)}` : '')
