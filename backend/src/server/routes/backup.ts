...
const progressStore = new Map<
    string,
    { percentage: number; message: string; startTime: number }
>();
...
async function getDiskSpace(
    ...
    if (
        diskSpaceCache &&
        now - diskSpaceCache.timestamp < DISK_SPACE_CACHE_TTL
    ) {
        return {
            available: diskSpaceCache.available,
            total: diskSpaceCache.total,
        };
    }
...
export function backup(app: any): void {
    ...
    app.post(
        ...
        async (req: Request) => {
            try {
                ...
                const { autoSchedule, scheduleCron, retentionDays } =
                    validation.data;
    ...
    app.get("/admin/backup/list", adminAuthMiddleware, async () => {
        try {
            const backups = await listBackups(
                env.backup_dir,
                env.backup_cloud_enabled || false,
                getS3Client(),
            );
        ...
        async (req: Request, ctx: any) => {
            try {
                ...
                if (location === "cloud") {
                    ...
                    const tempPath = join(
                        tmpdir(),
                        `restore-temp-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.db`,
                    );
                    ...
                    try {
                        ...
                        return new Response(
                            JSON.stringify({
                                success: true,
                                message:
                                    "Database restored successfully from cloud backup",
     ...
     */
    app.get(
        "/admin/backup/progress/:sessionId",
        adminAuthMiddleware,
        async (req: Request) => {
            try {
